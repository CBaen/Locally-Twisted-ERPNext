# Odoo Live Mirror — Site Inventory
Source: `http://5.78.136.133/` (failed Odoo test deployment)
Mirror: `_resources/odoo-live-mirror/pages/` — 346 HTML files
...
**Note:** `/contactus` (pages/contactus.html) is the same 50 KB contact page as `/contact` — Odoo's built-in `/contactus` route. Canonical is `/contact`. One page, two routes; only one needed in ERPNext.
...
```
/web/login          → pages/web_login.html        — Odoo auth — skip
/web/signup         → pages/web_signup.html       — Odoo auth — skip
/web/reset_password → pages/web_reset_password.html — Odoo auth — skip
```
...
**Odoo class conventions used:**
- `o_header_standard`, `o_colored_level`, `o_cc` — Odoo theme color-cascade
- `o_mega_menu`, `o_mega_menu_toggle` — Odoo mega menu engine
- `o_wsale_my_cart` — Odoo ecommerce cart widget
- `o_extra_menu_items` — Odoo overflow nav items
- `o_not_editable`, `oe_unremovable`, `oe_unmovable` — Odoo editor guards
- `s_text_block`, `oe_structure`, `oe_structure_solo` — Odoo snippet containers
...
**Newsletter JS** uses `js_subscribe` / `js_subscribe_value` — Odoo mailing list widget. ERPNext equivalent is a custom handler.

Odoo classes: `o_footer`, `js_subscribe`, `js_subscribe_btn`, `js_subscribe_value`.
...
3. **Confirmation modal:** Watches `window.location.hash` and `fetch` intercepts. If hash becomes `#received` OR fetch response URL ends with `/website/form/` + response is success, shows the confirmation modal overlay. Also listens for `hashchange` to handle Odoo's redirect-without-reload pattern.
...
```
Description text is in an Odoo editable `oe_structure` div — plain div content, not a tab.
...
**Price display:** `<span class="oe_price">$770.00</span>` — single price (not range). Updates client-side when size is changed via Odoo's `js_variant_change` JS. Base price = lowest variant.
...
**This page is a 404.** The product was unpublished (or deleted) before the crawl. The mirrored file contains Odoo's standard 404 page ("We couldn't find the page you're looking for!") with nav/footer. No product detail markup. The ERPNext rebuild has its own deposit handling — this reference is not needed.
...
Note: `/services` returns 404 in the crawl — the route exists on the Odoo site but the page was unpublished. It maps to the standard page template if content is added.
...
|---|---|---|
| `.svg` | 279 | Odoo UI icons (FontAwesome, OWL components, Odoo icon set). Do not copy. |
| (no extension / dynamic URLs) | 285 | Odoo dynamic endpoints (`/web/image/product.template/...`, `/web/static/...`). Product images served via Odoo's image controller — not static files. |
| `.webp` | 28 | **Product photos + variant swatch images** — see below |
| `.js` | 16 | Odoo bundle JS (assets_frontend_minimal, assets_frontend_lazy, etc.). Do not copy. |
| `.png` | 3 | LT logo + Odoo transparent placeholder + webshop placeholder thumbnail |
| `.jpg` | 3 | Hero slide backgrounds (slide_1.jpg, slide_2.jpg, slide_3.jpg) |
| `.css` | 2 | Odoo bundle CSS. Do not copy. |
| `.woff2 / .woff` | 4 | FontAwesome webfont. Do not copy — ERPNext ships its own FontAwesome. |

### Assets to copy verbatim into ERPNext app

These are genuine LT brand assets — not Odoo framework files:
...
1. **`/shop/event-booking-deposit-32` is a 404.** The deposit product (ID 32) was unpublished before the crawl. The 33 KB file is Odoo's generic 404 page. If a deposit flow is needed in ERPNext, it needs to be built fresh — there's no reference page here.

2. **Two "What We Make" top-level categories exist:** `/shop/category/what-we-make-3` (ID 3, paginated 3 pages) and `/shop/category/what-we-make-26` (ID 26, single page, 163 KB). The nav mega menu points at `/shop/category/what-we-make-3` as "Shop All Products." ID 26 is the `what-we-make` category shown as a nav item "What We Make" in the footer. These are two different category nodes in the Odoo tree that both cover the same product set — the ERPNext category tree will need a decision on this.
...
5. **`/contactus` duplicates `/contact`.** Odoo routes `/contactus` to its built-in contact controller. The crawled files are identical except for minor byte differences (50192 vs 50201 bytes). ERPNext does not have a `/contactus` route by default — a redirect `/contactus → /contact` is needed or the nav "Contact Us" button should point to `/contact` only.

6. **The nav "Contact Us" CTA button points to `/contactus` (the Odoo default route), not `/contact`.** All internal links in nav use `/contactus` for this button. The canonical contact page is `/contact`. ERPNext rebuild should standardize on `/contact`.

7. **Product page image carousels are single-image.** `data-image-amount="1"` on all crawled product pages. Multi-image carousels would require product images to be uploaded as extras in ERPNext — not present in this Odoo build.

8. **Color swatch images (`/web/image/product.attribute.value/<id>/image`)** are served as Odoo dynamic image URLs. These were not captured in the asset crawl. In ERPNext, `ItemAttributeValue` has a `colour` field for hex codes only — swatch images would require a custom field or a different approach (e.g., color-picker tiles).
