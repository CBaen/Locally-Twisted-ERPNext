...
- [P2] **One-time sweep of `scripts/verify/_screenshots/` accumulated bloat.** ~80 timestamped subdirs from sessions back to 2026-04-26 — most are dead diagnostic captures from work long-since shipped. The pattern that produced this: each instance creates oneshots and screenshots, often forgets to delete them, the next instance inherits the bloat. The 2026-04-29 evening session cleaned up its own mess (3 oneshot scripts + 12 temp dirs) but didn't touch prior accumulation. Cleanup options: (a) one-time sweep of all `_screenshots/*` except a curated keep-list, (b) `.gitignore` pattern that prevents commits going forward, (c) hard rule in CLAUDE.md operational rituals that every session must delete its own diagnostic captures at close. Per GL: *"GitHub is our archive, we store nothing unnecessary."*

- [P2] **Pre-existing Frappe asset-map bug** on `/shop/&lt;item&gt;` console: `Cannot read properties of undefined (reading 'file_uploader.bundle.js')`. Frappe's bundled `apps/frappe/frappe/public/js/frappe/upload.js:8` calls `bundled_asset('file_uploader.bundle.js')` against an undefined map entry. Page renders fine, no LT functionality affected. Investigate when there's bandwidth — likely a Frappe-upstream issue we report rather than fix locally.
