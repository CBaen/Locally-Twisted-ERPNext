...
- [P0] **Commerce rules and checkout policy follow-through.** Active lane at `workstreams/commerce-rules-checkout.md`; capability recipe at `.codex/capabilities/recipes/erpnext-checkout-commerce-rules.md`. Local checkout now separates tax rate lookup from taxable base: ready-to-order goods are taxable by ZIP/city rate, while services, service deposits, and delivery charges are non-taxable. Local/Park City delivery fees are supported; out-of-area delivery remains quote-required. Remaining: get GL/legal/accountant approval before final public Terms/Delivery/Refund/tax copy, keep future service/deposit Items in `Services` or explicitly non-taxable, and run live-mode payment readiness before cutover.
