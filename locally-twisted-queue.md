...
**Reconciliation note (2026-05-05):** `scripts/verify/layout_fit.spec.js` is restored and currently verifies through `npm run test:layout-fit` (260 passed). `npm run test:interactive-layout` adds 39 stateful public UI checks, `npm run test:checkout-experience` covers checkout preview behavior, and `npm run test:public-verify` is the broad public-site closeout gate. Treat `.planning/phases/01-customer-site-and-storefront/PLAN.md` as historical; `/contact` is the primary inquiry route and `/book` redirects to `/contact?intent=quick`.
...
- [P1] **Event Playground review and V1.1 hardening.** First PlayCanvas/Frappe route is hidden at `/event-playground`; feature lane is `workstreams/event-playground.md`. Next: review the local Vite iframe preview with GL/Jeff, decide whether it belongs in public nav, sales-shared only, or internal-only, then separately decide whether saved designs need a DocType, Frappe File screenshots, private share links, and Desk review. Keep backend saves, automatic Lead/Quote/Sales Order creation, pricing, checkout, room scanning, and full organic/twisting physics out of scope until explicitly approved.
