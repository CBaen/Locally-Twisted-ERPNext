...
**Reconciliation note (2026-05-06):** `scripts/verify/layout_fit.spec.js` is restored and currently verifies through `npm run test:layout-fit` (260 passed). `npm run test:interactive-layout` adds 42 stateful public UI checks, `npm run test:checkout-experience` covers checkout preview behavior, and `npm run test:website-verify` is the website-only closeout gate. Event Playground / PlayCanvas follow-up is parked for OpenClaw and is no longer part of the ASAP website launch lane. Treat `.planning/phases/01-customer-site-and-storefront/PLAN.md` as historical; `/contact` is the primary inquiry route and `/book` redirects to `/contact?intent=quick`.
