...
- [P0] **Final Civic visual QA and content/media audit.** Current V1 direction is implemented in `workstreams/civic-sitewide-redesign.md` and `_resources/STYLE-GUIDE.md`. Before launch, re-run route/layout/form/shop checks after any later content/media edits, inspect desktop/mobile screenshots, verify exact review/trust counts, and review gallery/category/product imagery with GL/Jeff. Do not reopen the old pastel/rainbow site identity unless GL explicitly reverses the Civic decision.
