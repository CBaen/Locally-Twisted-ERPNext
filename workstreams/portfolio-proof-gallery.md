...
Last updated: 2026-05-08 by Codex after GL rejected all photo captions,
visible frame wrappers, the old desktop scale, and the route-specific
Inquire/Studio/Index footer block.
...
visible frame wrappers, no forced aspect boxes, and no letterbox stripes. Mobile
must still feel like a moving collage stream instead of a static image stack.
...
- `scripts/verify/portfolio_reel.spec.js` verifies the current contract: no
  ...
  usage, fade/drift motion, click-to-front state, no captions, no frame
...
portfolio verifier passed with the footer guard. A clean Brave mobile check returned 200 with the
