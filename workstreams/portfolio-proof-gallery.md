...
`/portfolio` is the public proof gallery for LT's installed event work. The current route uses large natural-ratio photos in an edge-bleed floating reel instead of cropped card tiles. Captions stay in the DOM for accessibility/front-photo context, but they are hidden by default so text does not cover or compete with the photos. The photos carry the proof.
...
- `scripts/verify/portfolio_reel.spec.js` verifies desktop reel behavior, approved edge-anchored side/scale rhythm, scroll-driven reveal instead of a static row, optimized image usage, hidden-by-default captions, mobile full-width stacking, image aspect handling, query filtering, and empty-state layout.
...
Latest focused local verification on 2026-05-06: `/portfolio` returned 200; portfolio CSS, JS, and optimized images returned 200; portfolio reel 4/4 passed; layout-fit grep portfolio 13/13 passed; interactive-layout grep portfolio 3/3 passed. Fresh Chrome/Brave visual evidence from the running Frappe site is under `output/playwright/portfolio-strict-v5/`.

The strict v5 receipt specifically checked:

- edge anchors are restored so left/right photos can bleed past the viewport edge;
- Chrome and Brave render the same placement pattern;
- captions are not visible over the gallery photos unless a photo is brought forward;
- mobile starts with the portfolio hero and then becomes a full-width, natural-ratio photo stream.
