...
- Current local taxonomy state verified 2026-06-21 by `shop_taxonomy_contract.py` is 51 total Website Items, 47 published Website Items, 4 retired/unpublished needs-review Website Items, 8 visible primary groups under `Shop Items`, 9 secondary groups under hidden `Shop Occasions`, and 51 secondary Website Item Group rows (`47` attached to currently published products). Current sellability proof is 28 checkout Website Items and 19 quote-first Website Items. Re-check the target DB before changing seed logic or making staging/live claims from these numbers.
...
- Bulk catalog import lives in `seed_catalog.py` and honors captured catalog_data `data-attribute-exclusions`.
...
- Variant media first pass completed 2026-05-02: 1,712 variant `Item.image` values are set from `_resources/catalog-source/images/` where catalog_data image labels clearly matched product options. Product pages call `locally_twisted.api.variant_media.get_variant_media` after exact option selection and swap the main image when a variant image exists.
...
- Catalog media remains incomplete where the catalog_data image labels were too generic to map safely. Do not assign skipped images by guess; review them with GL/Jeff or add explicit mapping rules.
...
- Run the current catalog/source parity checks before layout overhaul work.
...
- Historical rebuild decisions: `locally-twisted-decisions.md` and
  `research/triadic-build-chrome-rebuild/`.
