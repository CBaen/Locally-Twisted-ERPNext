# Odoo Commerce Source Extraction + ERPNext/Frappe Ecommerce Parity/Safety Audit — Research Brief
...
Can Locally Twisted safely launch native ERPNext/Frappe ecommerce without losing the product-page depth, add-on behavior, customer intent, operator meaning, pricing logic, media meaning, cart/checkout behavior, and backend order/invoice/fulfillment data that the prior Odoo system either supported or exposed as necessary?
...
- Odoo is source witness/context, not infrastructure to copy. The target implementation must be native ERPNext/Frappe with secure fields, DocTypes, APIs, template overrides, pricing services, and fail-loud verifiers where needed.
...
1. Odoo source behavior extraction
...
- Copying Odoo code into Frappe.
- Treating Odoo data shape as automatically correct for ERPNext.
...
Must include:
- exact Odoo edition/version/module version evidence used for source claims, including whether evidence comes from source files, live/public pages, captured mirrors, exports, or production DB notes,
- product/page classes found in Odoo source,
...
### Lane E — Odoo Documentation / Agent-Action Convergence Researcher

Goal: compare what Odoo's official/user-facing ecommerce model says should happen with what agents observe by clicking through the Locally Twisted Odoo witness surfaces and current ERPNext/Frappe surfaces.

This lane is not allowed to copy Odoo code. It is allowed to read Odoo documentation, local source structure, public/live/captured pages, and current ERPNext/Frappe behavior to understand concepts, blast radius, and mismatch risk.

Required artifact:
`workstreams/ecommerce-audit/odoo-docs-agent-action-convergence-2026-05-10.md`

Must include:
- exact Odoo documentation pages/versions consulted,
- exact Odoo local/source/live/captured surfaces clicked or inspected,
- exact ERPNext/Frappe pages/actions clicked or inspected,
- a convergence table: documented behavior / observed Odoo behavior / observed ERPNext behavior / discrepancy / risk / required decision,
...
- discrepancies between Odoo documentation, Odoo observed behavior, and ERPNext observed behavior.
...
- Exact Odoo and ERPNext/Frappe versions are verified and cited before version-sensitive claims are trusted.
- Odoo code is not copied; Odoo is used only as a source witness.
