...
- includes a protected backend-generated Marketing Review Packet download
  generated at request time from approved public review inputs
- packet includes public sitemap and robots links plus the explicit no-indexing
  / no-Search-Console / no-ERPNext-editing boundary
...
- Do not attach private customer data, backend exports, files, invoices,
  orders, Leads, or product source records to the Marketing Review Packet. It
  is a sanitized public-site packet only.
...
Follow-up packet-access closeout from 2026-05-21:

- `/marketing-review` now shows a protected download link for the current
  Marketing Review Packet plus direct links to `/sitemap.xml` and `/robots.txt`.
- The packet is generated by
  `locally_twisted.marketing_review_access.download_marketing_review_packet`;
  it is not a stale uploaded file.
- The packet download requires the same explicit `LT Marketing Review Access`
  role as the review page and does not grant any new DocPerm rows or Desk
  access.
