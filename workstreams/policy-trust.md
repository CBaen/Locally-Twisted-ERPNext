...
Primary legacy_source business-source files:
...
|---|---|---|---|
| Privacy | Route exists. Page claims data collection, uses, sharing, analytics/advertising/tracking cookies, security, choices, children-under-13 online-data language, and `legal@locallytwisted.com` contact. | Questionnaire says Privacy Policy is launch-blocking and provides defaults/questions for data collected, tracking, selling, sharing, retention, privacy contact, marketing consent, children. GL business-proxy answers chose analytics/ads/tracking plus cart/session and narrow children-under-13 language; GL later split public inboxes so privacy/legal contact routes to `legal@locallytwisted.com`. | Business-approved for current copy; legal review still recommended. |
| Terms of Service | Route exists. Page claims booking confirmation rules, payment terms, cancellation reference, service area, weather, client responsibilities, website-use rules, `legal@locallytwisted.com` for terms/legal questions, and `billing@locallytwisted.com` for invoice/payment/AP questions. | Payment of invoice as signature, contract scope, deposits, Net 30, and 10% simple late fee trace to completed `live-interview-answers.md`. Broader ToS topics such as minimum age, customer uploads, dispute venue, class action, liability cap, warranty disclaimer, and change notices are in the unanswered questionnaire. | Partially sourced. Payment terms are strong; broader ToS legal terms need GL/legal approval or intentionally narrower launch language. |
...
| Accessibility | Route exists. Page says LT works to keep the site usable and uses `legal@locallytwisted.com`. | Questionnaire says Accessibility Statement exists and should be verified; default target is WCAG 2.1 AA. GL later split public inboxes so accessibility/legal copy routes to `legal@locallytwisted.com`. | Low-risk but not final. Needs GL/legal approval on whether to state WCAG target/features. |
