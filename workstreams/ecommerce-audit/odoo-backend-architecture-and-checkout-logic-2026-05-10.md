# Odoo Backend Architecture and Checkout Logic Source Witness

D:2026-05-10 | Check:live Odoo backend/public read-only inspection + local Odoo module source 2026-05-10 | Confidence:high

## Scope and guardrail

This is a read-only source-witness artifact. Odoo is being used as the behavioral reference for ERPNext/Frappe ecommerce receiving-layer design.
...
Sensitive checkout values observed in the browser were redacted from this artifact. Do not copy Odoo code/schema blindly; preserve business meaning and guardrails.

## Evidence sources

### Live Odoo backend / public site

- Backend product: `http://5.78.136.133/odoo/products/57` (`Classic Arch`).
- Public product: `http://5.78.136.133/shop/classic-arch-57`.
- Public cart, checkout, payment pages observed read-only.
- Live Odoo module: `locally_twisted`, installed version `19.0.2.15.0`.
...
- `C:\Users\baenb\projects\locally-twisted-odoo\addons\locally_twisted\__manifest__.py`
...
## 1. Odoo module architecture: this is not only ecommerce

The Odoo addon is a business system module, not just shop styling. Its manifest depends on website/shop, delivery, Stripe payments, portal/auth, CRM, calendar, account, purchase, MRP, project/sale_project, HR/timesheets/expenses, surveys, mass mailing, loyalty, base automation, and privacy lookup.
...
Critical pattern: **Odoo keeps the combinatorial dimension out of SKU variants**. Size creates the product identity; colors/design/lights are option payload attached to the cart/order line.
...
Odoo injects a product inquiry form below product description on every product page (`website_sale_templates.xml:114-123`).
...
- Delivery method options:
  - Standard delivery / free observed default label from Odoo native option,
  - Pickup (Free),
...
- Out-of-Area Quote: `delivery_data.xml:133`, fixed `$35` in current Odoo source.
...
Odoo uses sale/project/CRM links for fulfillment:
...
## 11. Public/security hardening observed in Odoo source
...
- Lane E convergence artifact is required in the packet; it should cite this live Odoo witness rather than the earlier artifactless child completion.
- This pass observed Odoo payment page but did not submit payment; no end-to-end payment claim is made.
- Existing current cart was observed read-only; this pass did not create a fresh cart/order.

## Bottom line

Odoo's architecture is not “every customer choice becomes a variant.” The survival pattern is:
...
## Addendum: deeper live Odoo extraction, 2026-05-10 14:05 MDT

D:2026-05-10 | Check:live Odoo JSON-RPC read-only extraction 2026-05-10T20:05Z | Confidence:high
...
This clears the specific stale `bench execute failed` blocker for now. It does not clear the no-payment-submission Odoo boundary and does not authorize product purge/import/public launch by itself.

---

## Addendum: catalog-wide backend pattern extraction, 2026-05-10 14:12 MDT

D:2026-05-10 | Check:live Odoo JSON-RPC catalog summary 2026-05-10T20:12Z | Confidence:high
...
Conclusion: Odoo's catalog architecture repeatedly uses the same pattern, not just Classic Arch. Variant axes stay small and commercially meaningful; large/customer-choice dimensions are no-variant option payloads.
...
D:2026-05-10 | Check:live Odoo public cart/checkout/payment DOM read-only 2026-05-10T20:13Z | Confidence:high
