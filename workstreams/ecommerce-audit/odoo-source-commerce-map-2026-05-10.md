D:2026-05-10 | Check:local Odoo source + public/browser Odoo surfaces 2026-05-10 | Confidence:[LOCAL-PROOF]

# Odoo Source Commerce Map — Locally Twisted

Purpose: map the working Odoo commerce meaning that ERPNext must model. This is a read-only source-witness artifact. It does **not** prescribe copying Odoo code into ERPNext.

## Scope and safety

- Read-only sources only: local Odoo repo, project dispatch/context files, and safe public/authenticated Odoo observations.
- No Odoo writes, saves, imports, purges, checkout submission, payment action, email send, or customer-data export were performed.
...
- `workstreams/ecommerce-audit/user-provided-odoo-surfaces-2026-05-10.md`.

### Local Odoo source witness

- `C:/Users/baenb/projects/locally-twisted-odoo/addons/locally_twisted/__manifest__.py` — module version `19.0.2.15.0`; dependencies include `website_sale`, `delivery`, `payment_stripe`, `sale_loyalty`.
...
- `deploy.py` — product/category/attribute/value/price-extra export/import shape; Odoo production is treated as authoritative for existing products.

### Odoo public/browser surfaces observed
...
## Current verified Odoo project state relevant to ecommerce

- Odoo witness repo branch checked as `main` during this lane.
- Local Odoo module version checked as `19.0.2.15.0`.
- The custom module depends on Odoo ecommerce/payment/delivery stack (`website_sale`, `payment_stripe`, `delivery`) and is therefore not just a static website theme.
- Odoo has a live/public shop at `http://5.78.136.133/shop` that rendered products, categories, filters, product pages, variant selectors, inquiry forms, and cart state during this audit.
- The Odoo shop had at least one cart item in the observed browser session. I did not inspect or change cart contents because that could reveal/session-mutate private state.

## Source commerce model: objects and meaning

### Product/page classes

Odoo represents purchasable commerce as `product.template` records, exposed by routes such as `/shop/classic-arch-57` and `/shop/unicorn-bouquet-115`.
...
Odoo uses product attributes/attribute values on `product.template` records. Local `deploy.py` exports:
...
|---|---|---|---|---|
| Arch Size | 20ft, 25ft, 30ft, 35ft | Classic Arch (`/shop/classic-arch-57`) | Effectively required as a single-choice variant axis; Odoo renders a default-selected radio group. | Classic Arch: 25ft `+ $65`, 30ft `+ $130`, 35ft `+ $195`; base 20ft at $260. |
...
| latex colors | Long palette including Reflex Champagne/Truffle/Silver/Gold/Blue/Green/Violet/Red, Dusk Cream/Green Tea/Blue/Lilac/Rose, Teal, Blue Slate, Smoke Grey, White, black, Red, Orange, yellow, raspberry, fuchsia, bubble Gum, eucalyptus, Forest, Shamrock, Wintergreen, Lime, LT Blue, Periwinkle, Royal Blue, Robin's Egg, Deep Teal, Honey, Violet, Orchid, Lilac, Chocolate, Brown, Latte, Pastels, Grey, Clear, Blush, Empowermint | Classic Arch, Classic Organic Garland, Number Balloon Columns, Balloon Drop | Multi-checkbox in Odoo; product copy says e.g. Classic Arch `up to 4 colors`, garlands `Select up to 4 colors`. | No price extras observed for latex color choices. |
...
- Browser product pages render Odoo variant controls and hidden `product_template_id` / `product_id` fields. One DOM sample for Classic Arch showed template ID `57` and selected product ID `91` with attribute line inputs such as `ptal-13` and `ptal-115`.
- Odoo product pages include combination/variant signals in HTML; product variants are computed by Odoo's native `website_sale` machinery.
...
- Multi-select color axes need max-count validation where product copy states limits (`up to 4 colors`, `up to 8 colors`), but I found that as page copy, not as a validated Odoo rule. Treat max-count enforcement as [UNKNOWN] until source/browser validation proves it.
...
Observed Odoo behavior suggests:

- Size/design axes rendered as single-choice radio/select groups are required by Odoo because one value must be selected for a valid combination; Odoo appears to default the first value.
- Multi-checkbox color axes are optional at the browser input level unless Odoo custom JS or server validation enforces a minimum; no such validation was found in local custom code. Product copy asks customers to select colors, but enforcement is [NO EVIDENCE].
...
- Variant-image switching is likely native Odoo `website_sale` behavior via `product.product` images, but I did not confirm a user-visible image changes when changing a variant. Mark variant-image switching as [UNKNOWN] until a safe browser interaction test captures before/after image URLs.
...
- Odoo shop pages include `Add to cart` for products like Unicorn Bouquet; public policy says `/shop` products are paid in full at checkout.
...
Source-backed Odoo concepts to model or consciously replace:
...
- [UNKNOWN] Full valid-combination matrix for every product. Odoo computes variants from product attribute lines; this artifact maps observed families and mechanics, not every combination.
...
- [UNKNOWN] Variant-specific image switching. Source and Odoo behavior imply product/product-template image support, but no before/after browser interaction was captured.
...
1. Run a read-only Odoo product export using the existing `deploy.py` export shape or equivalent safe server-side script, limited to product/category/attribute/value/price-extra/image metadata only. Do not include partners/leads/customer records.
...
6. Treat Odoo as source witness for commerce meaning, but verify ERPNext implementation against browser/test evidence before any purge/rebuild/import.

## Verification gate

- Verified local source by direct file reads and targeted `rg` searches.
- Verified public Odoo product/category/option behavior by browser snapshot and `web_fetch` on representative product routes.
- Verified no code changes/commits/product mutations were made during this lane.
- Artifact created at `workstreams/ecommerce-audit/odoo-source-commerce-map-2026-05-10.md`.
...
- Biggest rebuild risk: collapsing Odoo's product-specific attributes/price extras/multi-select add-ons into flat ERPNext Item records will lose working commerce behavior.
