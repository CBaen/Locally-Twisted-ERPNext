...
# Ecommerce Audit Handoff — catalog_data witness → native ERPNext receiving ecosystem

## Current peer state

This directory is the evidence packet for the May 10 ecommerce architecture audit. It is not a product import, not a destructive migration rehearsal, and not permission to copy catalog_data code. catalog_data is the source witness for business meaning; ERPNext/Frappe v15 receives that meaning through native DocTypes, custom fields, app services, templates, and verifiers.
...
`catalog-data-sellable-product-reimport-2026-05-17.md`: 53 checkout products, 0
...
`python scripts/verify/product_source_repair_map.py` from the catalog_data export,
...
|---|---|---|---|
| A — catalog_data source commerce map | `catalog_data-source-commerce-map-2026-05-10.md` | Present, read back | Cite for catalog_data source/product/page/option/pricing/media/cart meaning, with version mismatch labels. |
...
| E — catalog_data/docs/agent-action convergence | `catalog_data-docs-agent-action-convergence-2026-05-10.md` | Present, parent-created / recovered artifact-first | Cite for converged catalog_data witness → ERPNext receiving architecture, including no-variant options, cart intent, quote-first, checkout/payment boundary, automations, and verifier state. |
...
| Live catalog_data backend architecture witness | `catalog_data-backend-architecture-and-checkout-logic-2026-05-10.md` | Present, parent-created / live read-only backend + source proof | Direct catalog_data backend/public read-only observations: product/variant/no-variant architecture, 53-color behavior, cart/order-line preservation, checkout/payment boundary, delivery, CRM fields, automations, and ERPNext receiving requirements. |
| Infrastructure readiness packet | `ecommerce-infrastructure-readiness-packet-2026-05-10.md` | Present, parent-created | Current launch-readiness packet: what is proven, what remains blocked, catalog_data logic to preserve, verifier state, and next engineering gates. |
| ERPNext receiving build spec from catalog_data | `erpnext-receiving-build-spec-from-catalog_data-2026-05-10.md` | Present, parent-created | Converts live catalog_data backend logic into concrete ERPNext/Frappe object model, gaps, gates, and coding order. |
...
| Ready-to-order product cut plan | `ready-to-order-product-cut-plan-2026-05-10.md` | Historical, superseded by 2026-05-17 full import closeout | Earlier first-shelf plan; all 53 catalog_data products now target sellable checkout behavior locally unless GL explicitly excludes a product. |
...
| Product source repair map | `product-source-repair-map-2026-05-17.md` + `product-source-repair-map-2026-05-17.json` + `../../scripts/verify/product_source_repair_map.py` | Present, source-backed repair queue | Maps every catalog_data-export product to `purchasable_product`, reports 53/53 source rows found, and assigns the remaining 35 products to focused repair lanes instead of treating legacy holds as a product model. |
...
| User-provided catalog_data surfaces | `user-provided-catalog_data-surfaces-2026-05-10.md` | Present | Read-only surface references supplied by GL; do not click admin/auth surfaces without preflight. |
...
- catalog_data source witness local module is `addons/locally_twisted` `19.0.2.15.0`; prior handoff warns production DB may still be `19.0.2.14.0`, so source parity remains `[VERSION-MISMATCH]` until resolved.
...
1. Treat Lane A as present only through the parent-verified artifact `catalog_data-source-commerce-map-2026-05-10.md`; do not trust the earlier artifactless child completion event.
2. Use recovered Lane E (`catalog_data-docs-agent-action-convergence-2026-05-10.md`) as the convergence artifact; no longer carry Lane E as `[NO EVIDENCE]`.
3. Use `ecommerce-infrastructure-doc-map-and-synthesis-2026-05-10.md` as the front-door architecture map, `ecommerce-infrastructure-plan-v2-2026-05-10.md` as the active action plan, `ecommerce-infrastructure-readiness-packet-2026-05-10.md` for the current proof/gate packet, and `erpnext-receiving-build-spec-from-catalog_data-2026-05-10.md` for the concrete coding order; use product matrices only downstream of infrastructure gates.
...
7. Do not delete/reimport products, click admin-like catalog_data mutation paths, or mutate authenticated systems for this audit without a fresh rollback/preflight.
