...
# Ecommerce Audit Handoff — legacy_source witness → native ERPNext receiving ecosystem

## Current peer state

This directory is the evidence packet for the May 10 ecommerce architecture audit. It is not a product import, not a destructive migration rehearsal, and not permission to copy legacy_source code. legacy_source is the source witness for business meaning; ERPNext/Frappe v15 receives that meaning through native DocTypes, custom fields, app services, templates, and verifiers.
...
`legacy_source-sellable-product-reimport-2026-05-17.md`: 53 checkout products, 0
...
`python scripts/verify/product_source_repair_map.py` from the legacy_source export,
...
|---|---|---|---|
| A — legacy_source source commerce map | `legacy_source-source-commerce-map-2026-05-10.md` | Present, read back | Cite for legacy_source source/product/page/option/pricing/media/cart meaning, with version mismatch labels. |
...
| E — legacy_source/docs/agent-action convergence | `legacy_source-docs-agent-action-convergence-2026-05-10.md` | Present, parent-created / recovered artifact-first | Cite for converged legacy_source witness → ERPNext receiving architecture, including no-variant options, cart intent, quote-first, checkout/payment boundary, automations, and verifier state. |
...
| Live legacy_source backend architecture witness | `legacy_source-backend-architecture-and-checkout-logic-2026-05-10.md` | Present, parent-created / live read-only backend + source proof | Direct legacy_source backend/public read-only observations: product/variant/no-variant architecture, 53-color behavior, cart/order-line preservation, checkout/payment boundary, delivery, CRM fields, automations, and ERPNext receiving requirements. |
| Infrastructure readiness packet | `ecommerce-infrastructure-readiness-packet-2026-05-10.md` | Present, parent-created | Current launch-readiness packet: what is proven, what remains blocked, legacy_source logic to preserve, verifier state, and next engineering gates. |
| ERPNext receiving build spec from legacy_source | `erpnext-receiving-build-spec-from-legacy_source-2026-05-10.md` | Present, parent-created | Converts live legacy_source backend logic into concrete ERPNext/Frappe object model, gaps, gates, and coding order. |
...
| Ready-to-order product cut plan | `ready-to-order-product-cut-plan-2026-05-10.md` | Historical, superseded by 2026-05-17 full import closeout | Earlier first-shelf plan; all 53 legacy_source products now target sellable checkout behavior locally unless GL explicitly excludes a product. |
...
| Product source repair map | `product-source-repair-map-2026-05-17.md` + `product-source-repair-map-2026-05-17.json` + `../../scripts/verify/product_source_repair_map.py` | Present, source-backed repair queue | Maps every legacy_source-export product to `purchasable_product`, reports 53/53 source rows found, and assigns the remaining 35 products to focused repair lanes instead of treating legacy holds as a product model. |
...
| User-provided legacy_source surfaces | `user-provided-legacy_source-surfaces-2026-05-10.md` | Present | Read-only surface references supplied by GL; do not click admin/auth surfaces without preflight. |
...
- legacy_source source witness local module is `addons/locally_twisted` `19.0.2.15.0`; prior handoff warns production DB may still be `19.0.2.14.0`, so source parity remains `[VERSION-MISMATCH]` until resolved.
...
1. Treat Lane A as present only through the parent-verified artifact `legacy_source-source-commerce-map-2026-05-10.md`; do not trust the earlier artifactless child completion event.
2. Use recovered Lane E (`legacy_source-docs-agent-action-convergence-2026-05-10.md`) as the convergence artifact; no longer carry Lane E as `[NO EVIDENCE]`.
3. Use `ecommerce-infrastructure-doc-map-and-synthesis-2026-05-10.md` as the front-door architecture map, `ecommerce-infrastructure-plan-v2-2026-05-10.md` as the active action plan, `ecommerce-infrastructure-readiness-packet-2026-05-10.md` for the current proof/gate packet, and `erpnext-receiving-build-spec-from-legacy_source-2026-05-10.md` for the concrete coding order; use product matrices only downstream of infrastructure gates.
