D:2026-05-10 | Check:local catalog_data source + public/browser catalog_data surfaces 2026-05-10 | Confidence:[LOCAL-PROOF]

# catalog_data Source Commerce Map — Locally Twisted

Purpose: map the working catalog_data commerce meaning that ERPNext must model. This is a read-only source-witness artifact. It does **not** prescribe copying catalog_data code into ERPNext.

## Scope and safety

- Read-only sources only: local catalog_data repo, project dispatch/context files, and safe public/authenticated catalog_data observations.
- No catalog_data writes, saves, imports, purges, checkout submission, payment action, email send, or customer-data export were performed.
...
- `workstreams/ecommerce-audit/user-provided-catalog_data-surfaces-2026-05-10.md`.

### Local catalog_data source witness

- `/home/guidingl/projects/external-catalog-data/addons/locally_twisted/__manifest__.py` — module version `19.0.2.15.0`; dependencies include `website_sale`, `delivery`, `payment_stripe`, `sale_loyalty`.
...
- `deploy.py` — product/category/attribute/value/price-extra export/import shape; catalog_data production is treated as authoritative for existing products.

### catalog_data public/browser surfaces observed
...
## Current verified catalog_data project state relevant to ecommerce

- catalog_data witness repo branch checked as `main` during this lane.
- Local catalog_data module version checked as `19.0.2.15.0`.
- The custom module depends on catalog_data ecommerce/payment/delivery stack (`website_sale`, `payment_stripe`, `delivery`) and is therefore not just a static website theme.
- catalog_data has a live/public shop at `shop` that rendered products, categories, filters, product pages, variant selectors, inquiry forms, and cart state during this audit.
- The catalog_data shop had at least one cart item in the observed browser session. I did not inspect or change cart contents because that could reveal/session-mutate private state.

## Source commerce model: objects and meaning

### Product/page classes

catalog_data represents purchasable commerce as `product.template` records, exposed by routes such as `/shop/classic-arch-57` and `/shop/unicorn-bouquet-115`.
...
catalog_data uses product attributes/attribute values on `product.template` records. Local `deploy.py` exports:
...
|---|---|---|---|---|
| Arch Size | 20ft, 25ft, 30ft, 35ft | Classic Arch (`/shop/classic-arch-57`) | Effectively required as a single-choice variant axis; catalog_data renders a default-selected radio group. | Classic Arch: 25ft `+ $65`, 30ft `+ $130`, 35ft `+ $195`; base 20ft at $260. |
...
| latex colors | Long palette including Reflex Champagne/Truffle/Silver/Gold/Blue/Green/Violet/Red, Dusk Cream/Green Tea/Blue/Lilac/Rose, Teal, Blue Slate, Smoke Grey, White, black, Red, Orange, yellow, raspberry, fuchsia, bubble Gum, eucalyptus, Forest, Shamrock, Wintergreen, Lime, LT Blue, Periwinkle, Royal Blue, Robin's Egg, Deep Teal, Honey, Violet, Orchid, Lilac, Chocolate, Brown, Latte, Pastels, Grey, Clear, Blush, Empowermint | Classic Arch, Classic Organic Garland, Number Balloon Columns, Balloon Drop | Multi-checkbox in catalog_data; product copy says e.g. Classic Arch `up to 4 colors`, garlands `Select up to 4 colors`. | No price extras observed for latex color choices. |
...
- Browser product pages render catalog_data variant controls and hidden `product_template_id` / `product_id` fields. One DOM sample for Classic Arch showed template ID `57` and selected product ID `91` with attribute line inputs such as `ptal-13` and `ptal-115`.
- catalog_data product pages include combination/variant signals in HTML; product variants are computed by catalog_data's native `website_sale` machinery.
...
- Multi-select color axes need max-count validation where product copy states limits (`up to 4 colors`, `up to 8 colors`), but I found that as page copy, not as a validated catalog_data rule. Treat max-count enforcement as [UNKNOWN] until source/browser validation proves it.
...
Observed catalog_data behavior suggests:

- Size/design axes rendered as single-choice radio/select groups are required by catalog_data because one value must be selected for a valid combination; catalog_data appears to default the first value.
- Multi-checkbox color axes are optional at the browser input level unless catalog_data custom JS or server validation enforces a minimum; no such validation was found in local custom code. Product copy asks customers to select colors, but enforcement is [NO EVIDENCE].
...
- Variant-image switching is likely native catalog_data `website_sale` behavior via `product.product` images, but I did not confirm a user-visible image changes when changing a variant. Mark variant-image switching as [UNKNOWN] until a safe browser interaction test captures before/after image URLs.
...
- catalog_data shop pages include `Add to cart` for products like Unicorn Bouquet; public policy says `/shop` products are paid in full at checkout.
...
Source-backed catalog_data concepts to model or consciously replace:
...
- [UNKNOWN] Full valid-combination matrix for every product. catalog_data computes variants from product attribute lines; this artifact maps observed families and mechanics, not every combination.
...
- [UNKNOWN] Variant-specific image switching. Source and catalog_data behavior imply product/product-template image support, but no before/after browser interaction was captured.
...
1. Run a read-only catalog_data product export using the existing `deploy.py` export shape or equivalent safe server-side script, limited to product/category/attribute/value/price-extra/image metadata only. Do not include partners/leads/customer records.
...
6. Treat catalog_data as source witness for commerce meaning, but verify ERPNext implementation against browser/test evidence before any purge/rebuild/import.

## Verification gate

- Verified local source by direct file reads and targeted `rg` searches.
- Verified public catalog_data product/category/option behavior by browser snapshot and `web_fetch` on representative product routes.
- Verified no code changes/commits/product mutations were made during this lane.
- Artifact created at `workstreams/ecommerce-audit/catalog_data-source-commerce-map-2026-05-10.md`.
...
- Biggest rebuild risk: collapsing catalog_data's product-specific attributes/price extras/multi-select add-ons into flat ERPNext Item records will lose working commerce behavior.
