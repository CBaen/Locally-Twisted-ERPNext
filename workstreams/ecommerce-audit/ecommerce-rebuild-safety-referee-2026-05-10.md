...
- `AGENTS.md` — project operating rules, current verified state, launch trust rules, Odoo-as-source-witness framing.
...
1. **Lane A — Odoo source commerce map**
   - Required artifact: `workstreams/ecommerce-audit/odoo-source-commerce-map-2026-05-10.md`.
   - Must enumerate current Odoo/source product meaning: product classes, variant axes, valid combinations, add-ons, customizations, price sources/resolvers, media facts, quote behavior, unknowns, source version, and production/local mismatch.
   - Current status: missing / `[NO EVIDENCE]` per `README.md`.
   - Safety impact: no source-product claim should be cited as final.

2. **Lane E — Odoo/docs/agent-action convergence**
   - Required artifact: `workstreams/ecommerce-audit/odoo-docs-agent-action-convergence-2026-05-10.md`.
   - Must reconcile Odoo witness behavior, ERPNext/Frappe/Webshop docs/current behavior, project capabilities, and actual agent actions into one conflict map.
   - Current status: missing / `[NO EVIDENCE]` per `README.md`.
...
3. **Version reconciliation**
   ...
   - Lane B also reports Odoo local module `19.0.2.15.0` while older handoff warns production DB may be `19.0.2.14.0`.
...
- Which Odoo/source witness version controls the rebuild (`19.0.2.15.0` local vs possible `19.0.2.14.0` production DB).
...
|---|---|
| Lane A Odoo source map | Complete enough to name source product meaning, unknowns, and version mismatch; otherwise `[NO EVIDENCE]` and blocks source claims. |
...
2. Rerun **Lane E** artifact-first; reconcile Odoo/source, ERPNext/Frappe docs/current behavior, project capabilities, and agent actions.
3. Resolve/sign the **version witness decision** for Odoo source and ERPNext destination image/app versions.
