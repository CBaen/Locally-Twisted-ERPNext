...
- `workstreams/ecommerce-audit/README.md` says Lane B/C/D artifacts exist, but Lane A Odoo source map and Lane E docs/action convergence are missing and must be `[NO EVIDENCE]`.
...
- `workstreams/ecommerce-audit/native-frappe-product-template-architecture-2026-05-10.md` recommends a native ERPNext/Frappe contract layer, not Odoo code copying.
- `audits/odoo-erpnext-migration-audit-2026-05-08/15-product-page-contract-source-audit.md` classifies 53 source products as 15 ready-to-order and 38 quote-first, but says destructive purge/import is blocked.
...
1. **Lane A Odoo source map is missing** — no named artifact exists for refreshed Odoo source product behavior, source axes, photos, option-changing photos, cart/quote behavior, and backend sales surfaces.
2. **Lane E docs/action convergence is missing** — no named artifact reconciles Odoo docs/source/admin surfaces with current agent action plan.
...
8. **Version/source mismatch remains unresolved** — destination image label and Odoo local/prod module version mismatch must be resolved or explicitly accepted before import claims.
...
## Questions Odoo source mapping must answer

Lane A must produce a named artifact that answers, product-by-product:

1. What is the canonical Odoo source product/template/variant identity, and which ERPNext Website Item should receive it?
...
6. What is the price source for each sale unit: Odoo resolver, Odoo base/list price, live ERPNext snapshot, or human-approved override?
...
9. What did Odoo show in product/sales/backend admin surfaces that ERPNext must preserve for Jeff/operator meaning?
10. What source version controlled the answer: local Odoo `19.0.2.15.0`, possible production `19.0.2.14.0`, live shop scrape, or GL/Jeff decision?
...
1. **Rerun Lane A artifact-first** using Odoo source/admin/product/sales surfaces; no writes, no code copying, no secrets.
2. **Rerun Lane E artifact-first** to reconcile Odoo source/docs/admin observations with ERPNext-native action plan.
...
- Any statement that full Odoo source parity is known while Lane A is missing.
