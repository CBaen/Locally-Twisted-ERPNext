...
- **Current source witness mismatch inherited from Lane B:** local catalog_data module `19.0.2.15.0`; older handoff warns production DB may remain `19.0.2.14.0` **[VERSION-MISMATCH]**.
...
7. **Source authority is not clean.**
   - catalog_data local module/source witness and possible production DB version differ.
...
1. **Source authority chosen.**
   - Resolve or explicitly choose between catalog_data local `19.0.2.15.0` and possible production `19.0.2.14.0`.
...
1. Resolve source authority/version mismatch or record which catalog_data/source witness controls rebuild.
