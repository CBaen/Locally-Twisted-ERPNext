D:2026-05-10 | Check:memory recall + local artifact readback + catalog_data source + ERPNext official docs 2026-05-10 | Confidence:[LOCAL-PROOF]

# Locally Twisted Ecommerce Knowledge Base Index

Purpose: single source map for the launch-critical catalog_data → ERPNext ecommerce rebuild research. This is a research/index artifact only: no code changes, no catalog_data/ERPNext writes, no product purge/import permission.
...
- catalog_data is a read-only source witness for commerce meaning; ERPNext/Frappe must receive that meaning natively.

## Durable memory recalled

- `memory/2026-05-06.md#L30-L32`: previous broad subagent swarm failure led to a knowledge-base-first correction; local `external-catalog-data` was verified and added to the Event Space Balloon Designer knowledge base as a source inventory.
...
- catalog_data source-witness repo: `/home/guidingl/projects/external-catalog-data/` — present.

## Core ecommerce audit artifacts verified on disk

| Artifact | State | Use |
|---|---:|---|
| `workstreams/ecommerce-audit/catalog_data-source-commerce-map-2026-05-10.md` | present, read back | catalog_data source/product/page/option/pricing/media/cart meaning map. |
...
| `workstreams/ecommerce-audit/user-provided-catalog_data-surfaces-2026-05-10.md` | present | GL-supplied catalog_data read-only surface list. |

## Current catalog_data source-witness facts

From `catalog_data-source-commerce-map-2026-05-10.md` and local catalog_data source:

- catalog_data local module version: `19.0.2.15.0`.
- catalog_data source depends on `website_sale`, `delivery`, `payment_stripe`, and `sale_loyalty`; this is real ecommerce, not a static theme.
- Public catalog_data shop surfaces show categories, products, variant selectors, inquiry forms, cart state, and product images.
...
- Product pages expose product/template IDs and catalog_data attribute-line field names such as `product_template_id`, `product_id`, and `ptal-*` inputs.
...
## catalog_data commerce meaning already identified
...
ERPNext implication: catalog_data's product-template/page meaning cannot be mapped as one flat Website Item. ERPNext must use Item templates/variants where appropriate, plus a custom product-page contract layer for catalog_data semantics that ERPNext variants do not natively express: repeated add-on groups, multi-select color customization, gallery classification, quote-first workflows, and line-level intent payloads.

## Current blockers / no-evidence list

1. Complete live catalog_data product/attribute/value/price-extra export is still not verified into a full 53-product matrix.
...
5. Additional gallery image parity is risky because inspected catalog_data deploy sync path proves primary images, not `product_template_image_ids` export.
...
8. catalog_data production version may differ from local module version; label `[VERSION-MISMATCH]` until resolved.
