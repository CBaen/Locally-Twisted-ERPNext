D:2026-05-10 | Check:memory recall + local artifact readback + Odoo source + ERPNext official docs 2026-05-10 | Confidence:[LOCAL-PROOF]

# Locally Twisted Ecommerce Knowledge Base Index

Purpose: single source map for the launch-critical Odoo → ERPNext ecommerce rebuild research. This is a research/index artifact only: no code changes, no Odoo/ERPNext writes, no product purge/import permission.
...
- Odoo is a read-only source witness for commerce meaning; ERPNext/Frappe must receive that meaning natively.

## Durable memory recalled

- `memory/2026-05-06.md#L30-L32`: previous broad subagent swarm failure led to a knowledge-base-first correction; local `locally-twisted-odoo` was verified and added to the Event Space Balloon Designer knowledge base as a source inventory.
...
- Odoo source-witness repo: `C:\Users\baenb\projects\locally-twisted-odoo` — present.

## Core ecommerce audit artifacts verified on disk

| Artifact | State | Use |
|---|---:|---|
| `workstreams/ecommerce-audit/odoo-source-commerce-map-2026-05-10.md` | present, read back | Odoo source/product/page/option/pricing/media/cart meaning map. |
...
| `workstreams/ecommerce-audit/user-provided-odoo-surfaces-2026-05-10.md` | present | GL-supplied Odoo read-only surface list. |

## Current Odoo source-witness facts

From `odoo-source-commerce-map-2026-05-10.md` and local Odoo source:

- Odoo local module version: `19.0.2.15.0`.
- Odoo source depends on `website_sale`, `delivery`, `payment_stripe`, and `sale_loyalty`; this is real ecommerce, not a static theme.
- Public Odoo shop surfaces show categories, products, variant selectors, inquiry forms, cart state, and product images.
...
- Product pages expose product/template IDs and Odoo attribute-line field names such as `product_template_id`, `product_id`, and `ptal-*` inputs.
...
## Odoo commerce meaning already identified
...
ERPNext implication: Odoo's product-template/page meaning cannot be mapped as one flat Website Item. ERPNext must use Item templates/variants where appropriate, plus a custom product-page contract layer for Odoo semantics that ERPNext variants do not natively express: repeated add-on groups, multi-select color customization, gallery classification, quote-first workflows, and line-level intent payloads.

## Current blockers / no-evidence list

1. Complete live Odoo product/attribute/value/price-extra export is still not verified into a full 53-product matrix.
...
5. Additional gallery image parity is risky because inspected Odoo deploy sync path proves primary images, not `product_template_image_ids` export.
...
8. Odoo production version may differ from local module version; label `[VERSION-MISMATCH]` until resolved.
