...
Odoo witness repo: `C:\Users\baenb\projects\locally-twisted-odoo`
...
- Source witness: Locally Twisted Odoo 19 Community, local module `addons/locally_twisted` currently `19.0.2.15.0`; prior notes warn production DB may be `19.0.2.14.0`.
...
- You must not send real customer email, submit live payment, change DNS, deploy, push commits, approve prices/add-ons, delete/reimport products, mutate Odoo, or touch real customer data except minimal read-only inspection if unavoidable and explicitly noted.
- Odoo is read-only source witness and conceptual teacher. Do not copy Odoo code into ERPNext/Frappe. We are translating business meaning, not stealing implementation.

Stop and report immediately if you hit: real payment path, real customer data mutation risk, secrets exposure, Odoo write temptation, product deletion/reimport temptation, unsupported add-on/variant accepted visually, backend customer intent loss, or version/source/live/docs disagreement that changes conclusions.
...
Task:
Extract source commerce meaning from the Odoo witness without proposing ERPNext implementation first.

Required artifact:
`workstreams/ecommerce-audit/odoo-source-commerce-map-2026-05-10.md`

Must include:
- exact Odoo edition/version/module evidence used, and whether each claim comes from source file, docs, public/live page, captured mirror, export, or production DB note;
...
Task:
Compare Odoo documentation, Odoo observed behavior, and ERPNext/Frappe observed behavior. Find discrepancies and risks.
...
- convergence table: documented behavior / observed Odoo behavior / observed ERPNext behavior / discrepancy / risk / required decision;
...
Do not copy Odoo code. Use docs and observations to translate concepts.
