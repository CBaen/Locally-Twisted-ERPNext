...
3. **Access boundaries are explicit**
   ...
   - Agents must not send real customer emails, submit live payment, change DNS, publish/deploy, push commits, mutate production customer records, approve prices/add-ons, delete products, reimport products, or write to the Odoo repo.
   - Odoo is read-only source witness. ERPNext/Frappe is the native destination.
...
5. **Documentation convergence lane exists**
   One agent must compare Odoo docs, Odoo observed behavior, ERPNext docs/source, and ERPNext observed behavior. Discrepancies must be reported, not smoothed over.
...
5. Odoo Documentation / Agent-Action Convergence Researcher
6. Referee / Synthesis, run last only after lane artifacts exist

## Required phrasing in every prompt

- We are translating business meaning, not copying Odoo code.
- Odoo is mature source witness and conceptual teacher, not implementation to steal.
