...
- Exact browser/API checks confirmed `/checkout?item=6-color-rainbow-arch-20F&qty=1` returns 200, `/checkout?item=6-color-rainbow-arch&qty=1` is blocked, and product-page option selection writes variant `6-color-rainbow-arch-20F` into `LT_CART`.
...
- Product option UX P0 pass completed 2026-05-02: no per-attribute Jinja DB lookup, progressive invalid-option disabling is verified, and variant chips are radio/single-select.
