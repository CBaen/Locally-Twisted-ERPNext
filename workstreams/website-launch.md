...
Last updated: 2026-05-07 by Codex after the executable public container contract, compact hero contract repair, and homepage/nav proof repair.
...
- BTFP form/calculator follow-up completed 2026-05-08. The event crawl is now brand-blue instead of Berry, the page includes a customer-facing artist-time calculator using the published $130 first hour / $115 additional hour / $50 deposit per artist math, and the shared public inquiry partial now declares `data-form-contract="inquiry-v1"`. The durable form visual contract is in `_resources/STYLE-GUIDE.md` and `locally-twisted-decisions.md`. `contact_prefill.py` now guards the BTFP calculator, minimum one-hour rule, no public deposit checkout CTA, BTFP-only service choices, and the shared form contract.
...
- Responsive container integrity gate completed 2026-05-05 and was refreshed after the 2026-05-07 BTFP/Process correction, compact hero repair, and executable route-level container contract. `npm run test:layout-fit` now checks the current public route list across 13 viewport families (247 checks after `/process` removal), `npm run test:container-contract` checks route-level container ownership across 57 route/viewport cases, `npm run test:interactive-layout` checks 88 stateful UI cases, and `npm run test:public-verify` aliases the full website-only closeout gate.
