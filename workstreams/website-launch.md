...
is pages/forms first with ecommerce hidden by `lt_ecommerce_paused=1`.
Ready-to-Order, shop, product, cart, and checkout work is preserved and can be
...
can understand the offer, review proof, submit inquiries, and reach clear
policy/contact paths without broken routes, inaccessible layouts, stale docs, or
missing policy basics. If ecommerce is hidden, shop/cart/checkout routes must
show the branded quote fallback instead of broken or half-live commerce.
...
2026-05-10 pages/forms-first launch update: ecommerce is currently hidden in
...
snapshot from that run was removed during launch cleanup; rerun the verifier or
snapshot command when fresh local evidence is needed.
The launch verifier waits for localhost before starting, and it retries once if
...
The prepared-design proof copy and delivery/truck icon are removed.
Ready-to-Order source chrome is config-gated: hidden in the current
pages/forms-first launch posture and available again when ecommerce is
reopened. Coordination:
...
pages/forms-first hidden-commerce launch gate passed all 15 steps with
accessibility and contact smoke included.
...
`python scripts/verify/ecommerce_pause_contract.py` passed. GL later reopened
public ecommerce testing, then clarified the day-of-launch priority as
pages/forms first with ecommerce optional. The pause proof is now the current
safe launch fallback.

2026-05-10 SEO/GEO/AEO closeout: `npm run test:seo-contract` now guards
canonical aliases, sitemap canonical routes plus ecommerce URLs,
...
`capabilities/recipes/frappe-public-storefront-security.md`. First safe
...
- `scripts/verify/category_media_candidates.py` now generates a no-mutation approval packet for the 8 active customer-facing category images. Latest taxonomy smoke on 2026-05-24 wrote ignored local reports to `output/category-media-candidates-taxonomy-smoke.json` and `output/category-media-candidates-taxonomy-smoke.md`; the old 11-category packet is historical.
- `scripts/setup/sync_category_media.py` handles the post-approval path: write a selection template, dry-run Frappe Item Group image updates, and apply only rows marked `approved: true`. Safety proof on 2026-05-06 made 0 updates with an unapproved template; current Item Group image assignment remains unapproved.
...
- Nav/BTFP correction completed 2026-05-07 and refreshed 2026-05-11. Handoff: `workstreams/nav-btfp-process-correction.md`; removed hub handoff: `workstreams/event-balloons-route-removal-2026-05-11.md`. It restored `/balloon-twisting-and-face-painting` as a 200 service route, removed the unapproved `/process` route, and removed the unapproved `/event-balloons` hub route with no redirect. The current open-ecommerce header uses a non-link `Event Balloons` audience dropdown, `Twisting & Face Painting`, `Ready-to-Order`, `Portfolio`, `About Us`, `FAQ`, and `Contact Us`, with Ready-to-Order pointing to `/shop`, Contact Us pointing to `/contact`, event audience links pointing to the four audience pages, top-banner `Free Event Quote` also pointing to `/contact`, the 24-hour short-notice message as a deep-navy linked desktop/mobile banner, and the old proof copy/truck icon removed.
...
- `npm run test:layout-fit` passed with 325 checks across the current public route list.
- `npm run test:container-contract` passed with 75 route/viewport checks across the launch public routes.
...
- `python scripts/verify/smoke_shop.py` passed on 2026-05-10 in open ecommerce mode before the launch posture changed: it verifies open shop/category/product-detail checks, current BTFP/quote/contact chrome, Ready-to-Order, retired `/search`, optional add-on checkbox distinction, variant add-to-cart, and mobile drawer behavior. Current launch proof uses `python scripts/verify/ecommerce_pause_contract.py` plus `python scripts\verify\website_launch_verify.py --with-a11y --with-contact-smoke`, with `/shop`, `/cart`, and `/checkout` covered by the branded pause fallback.
- `nav_ia.py` now verifies the desktop/mobile non-link event audience dropdown, `Twisting & Face Painting`, `Ready-to-Order`, `Portfolio`, `About Us`, `FAQ`, `Contact Us`, and top-banner `Free Event Quote`/linked short-notice/account chrome when ecommerce is open. It also fails if `/event-balloons` route files, links, route rules, or canonical mappings return. In the current hidden-ecommerce launch posture, `Ready-to-Order` and cart are source-gated out while the deep-navy desktop/mobile short-notice strips, desktop centering, and visible mobile notice remain guarded.
...
- Historical layout-fit rerun found and fixed a 320px overflow on the old seasonal category route; Webshop's stock `.item-card { min-width: 300px; }` needed the LT grid override `min-width: 0`. Current active category routing is governed by the 2026-05-24 taxonomy proof.
...
- Responsive container integrity gate completed 2026-05-05 and was refreshed after the 2026-05-07 BTFP/Process correction, compact hero repair, executable route-level container contract, 2026-05-08 manual accessibility closeout, 2026-05-10 generated hero correction, and 2026-05-10 BTFP crawl/header update. `npm run test:layout-fit` now checks the current public route list across 13 viewport families (325 checks), `npm run test:container-contract` checks route-level container ownership across 75 route/viewport cases, `npm run test:interactive-layout` checks 163 stateful UI cases, `npm run test:a11y-manual` checks public-route keyboard focus/image/zoom-pressure exposure, and `npm run test:public-verify` aliases the full website-only closeout gate.
...
- `smoke_shop.py` now matches the current commerce mode. In open-commerce mode, it owns fixed-price product pages, retail variant option selection/cart writes, quote-first gates, optional add-on selector boundaries, and shop/category showroom contracts. In pause mode, it verifies the public pause and skips open-shop card/rail/product-detail checks.
...
- browse shop/product surfaces and exercise cart/checkout in the local testing path without unstable or fake-success behavior
...
### Gate 4 - Ecommerce Full Testing

Launch blocker if public ecommerce is open but customer paths are not actually verified:

- Public chrome must expose Ready-to-Order/cart/search consistently when `lt_ecommerce_paused=0`.
- Guest `/shop`, `/shop-items`, `/shop-by-category`, `/all-products`, `/cart`, and `/checkout` traffic must render the real customer ecommerce path.
- Shop/product/cart/checkout tests must prove category navigation, variant selection, add-to-cart, cart display, checkout form visibility, fulfillment rules, and checkout-to-Lead/backend cascades.
- If the pause switch is later re-enabled, the branded pause contract must be re-run and docs updated in the same slice.
...
|---|---|---|---|
| Controller baseline | Passing after current pages/forms-first gate | `website_launch_verify.py --with-a11y --with-contact-smoke` passed 15/15 in hidden-commerce mode: route/layout/container/interactive checks, `a11y` 50 route/viewport results with 0 violations, `a11y-manual`, contact smoke with backend record proof and cleanup, portfolio 6/6, pause contract, and `smoke_shop.py` in pause mode all passed | Keep this file current after every lane return |
...
| Reindex / provider cleanup | Source fix started; live not ready for Search Console submit; selective indexing gate added locally | `workstreams/domain-provider-reindex-cleanup-2026-05-19.md`, `workstreams/seo-geo-aeo-contract.md`, `workstreams/selective-indexing-gate-2026-05-21.md`, `capabilities/recipes/lt-seo-geo-aeo-contract.md`, and `capabilities/failures/frappe-cloud-sitemap-public-domain-drift.md` | Release sitemap/canonical/selective-indexing fix, rerun live SEO contract, confirm stable pages index while ecommerce discovery stays noindex during pause, then submit approved sitemap/priority URLs in Search Console |
...
- Project capability: `capabilities/recipes/take-live-coordinated-workflows.md`.
...
- Take-live coordination recipe: `capabilities/recipes/take-live-coordinated-workflows.md`.
