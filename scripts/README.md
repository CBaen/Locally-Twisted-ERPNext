...
| `paperwork_review_digest.py` | Read-only internal digest combining paperwork status, automation index, unpaid invoice review, and draft packets into one review payload. | Before creating a Desk queue or scheduled internal digest and after changing paperwork review surfaces |
...
| `customer_reminder_review_report.py` | No-live customer reminder report data source. Turns dry-run queue items into table rows, review/hold groups, and optional JSON/Markdown/CSV output without customer delivery. | Before building a Desk page, internal report, or scheduled internal-only reminder review surface |
