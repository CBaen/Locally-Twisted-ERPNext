...
| `paperwork_status.py` | Read-only paperwork/backend status report for invoices, payment requests, email queues, live payment blockers, and setup gaps. Outputs optional JSON to ignored `output/`. | When reviewing back-office launch readiness before adding finance automation |
