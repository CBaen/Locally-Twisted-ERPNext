#!/usr/bin/env python3
"""
Re-categorize the 33 seeded Items into 4 Item Groups per GL 2026-04-27 r4.

Three groups for the Quick Picks shop (filter pills on /quick-picks):
...
# Quick Picks vs Event Decor distinction.
ASSIGNMENTS = {
...
QUICK_PICKS_GROUPS = {"Bouquets", "Columns", "Ready-Made"}
...
def reassign_items(opener):
    ...
    for slug, target_group in ASSIGNMENTS.items():
        ...
        if current_group != target_group:
            ...
            if target_group in QUICK_PICKS_GROUPS:
                moved_q += 1
            else:
                moved_e += 1
        else:
    ...
    print(
        f"\n[summary] Moved to Quick Picks: {moved_q} / Moved to Event Decor: {moved_e} / "
...
def main():
    ...
    print()
    print(f"OK -> {BASE}/quick-picks (page TBD)")
    print(f"OK -> {BASE}/all-products (still works; now shows only Quick Picks)")
    return 0
