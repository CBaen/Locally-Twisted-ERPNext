#!/usr/bin/env python3
"""
Translate Locally Twisted's Odoo `lt.dashboard.review` model into a Frappe
Custom DocType in the local LT ERPNext install at http://localhost:8081.

Source: locally-twisted-odoo/addons/locally_twisted/models/dashboard_review.py
Target: http://localhost:8081/app/doctype/dashboard-reviewed-item

This is the first model translation in the LT Odoo->ERPNext migration.
...
# Field schema mirroring Odoo's lt.dashboard.review with Frappe-native types.
# Notes inline explain divergences from the Odoo source.
FIELDS = [
    {
        "fieldname": "user",
        "label": "User",
        "fieldtype": "Link",
        "options": "User",                  # Frappe's User DocType ~ Odoo res.users
        "reqd": 1,
    ...
    },
    {
        ...
        "reqd": 1,
        "in_list_view": 1,
        "description": "Frappe records are name-keyed strings, not integer IDs like Odoo's source_res_id.",
    },
    {
    ...
    payload = {
        "doc": {
            ...
            "description": (
                ...
                "Translated from Odoo lt.dashboard.review on 2026-04-25 by Trellis. "
                "TODO: add a Server Script to force user = session.user on insert "
                "(Odoo's create() override prevents client-side user_id spoofing)."
            ),
            "fields": FIELDS,
            "permissions": [
                {
