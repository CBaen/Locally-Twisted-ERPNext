#!/usr/bin/env python3
"""Verify fake quote/proposal draft packet scenarios.

Run:
  python scripts/verify/quote_proposal_draft_packet_contract.py
  python scripts/verify/quote_proposal_draft_packet_contract.py --json
"""
...
SITE = "frontend"
METHOD = "locally_twisted.verify.quote_proposal_draft_packet_contract.run"


class QuoteProposalDraftPacketContractFail(Exception):
    pass
...
def bench_execute() -> dict[str, Any]:
    ...
    if proc.returncode != 0:
        raise QuoteProposalDraftPacketContractFail(
            f"bench execute failed\nSTDOUT:\n{proc.stdout}\nSTDERR:\n{proc.stderr}"
        )
    text = proc.stdout.strip()
    if not text:
        raise QuoteProposalDraftPacketContractFail("quote/proposal draft packet contract returned no output")
    try:
        parsed = json.loads(text)
    except json.JSONDecodeError as exc:
        raise QuoteProposalDraftPacketContractFail(
            f"quote/proposal draft packet contract returned non-JSON output: {text}"
        ) from exc
    if not isinstance(parsed, dict):
        raise QuoteProposalDraftPacketContractFail(
            f"quote/proposal draft packet contract returned {type(parsed).__name__}, expected object"
        )
    return parsed
...
def main() -> int:
    ...
    except QuoteProposalDraftPacketContractFail as exc:
        print(f"[QUOTE PROPOSAL DRAFT PACKET CONTRACT] FAIL\n  - {exc}")
        return 1
...
def _print_summary(result: dict[str, Any], contract_failures: list[str]) -> None:
    print(
        "[QUOTE PROPOSAL DRAFT PACKET CONTRACT] "
