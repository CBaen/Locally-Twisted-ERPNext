#!/usr/bin/env python3
"""Verify fake no-live customer reminder review report scenarios.

Run:
  python scripts/verify/customer_reminder_review_report_contract.py
  python scripts/verify/customer_reminder_review_report_contract.py --json
"""
...
SITE = "frontend"
METHOD = "locally_twisted.verify.customer_reminder_review_report_contract.run"


class CustomerReminderReviewReportContractFail(Exception):
    pass
...
def bench_execute() -> dict[str, Any]:
    ...
    if proc.returncode != 0:
        raise CustomerReminderReviewReportContractFail(
            f"bench execute failed\nSTDOUT:\n{proc.stdout}\nSTDERR:\n{proc.stderr}"
        )
    text = proc.stdout.strip()
    if not text:
        raise CustomerReminderReviewReportContractFail("customer reminder review report contract returned no output")
    try:
        parsed = json.loads(text)
    except json.JSONDecodeError as exc:
        raise CustomerReminderReviewReportContractFail(
            f"customer reminder review report contract returned non-JSON output: {text}"
        ) from exc
    if not isinstance(parsed, dict):
        raise CustomerReminderReviewReportContractFail(
            f"customer reminder review report contract returned {type(parsed).__name__}, expected object"
        )
    return parsed
...
def main() -> int:
    ...
    except CustomerReminderReviewReportContractFail as exc:
        print(f"[CUSTOMER REMINDER REVIEW REPORT CONTRACT] FAIL\n  - {exc}")
        return 1
...
def _contract_failures(result: dict[str, Any]) -> list[str]:
    ...
    expected = {
        "mixed_queue_groups_rows",
        "empty_queue_report_ok",
        "malformed_send_enabled_rejected",
    }
...
def _print_summary(result: dict[str, Any], contract_failures: list[str]) -> None:
    print(
        "[CUSTOMER REMINDER REVIEW REPORT CONTRACT] "
