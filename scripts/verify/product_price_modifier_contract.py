...
HOST_SOURCE_ROOT = ROOT / "_resources" / "odoo-live"
...
CONTAINER_SOURCE_ROOTS = (
    ...
    "/workspace/_resources/odoo-live",
    "/home/frappe/frappe-bench/_resources/odoo-live",
    "/home/frappe/frappe-bench/sites/_resources/odoo-live",
)
...
def _missing_source_hint(text: str) -> bool:
    lowered = text.lower()
    return "_resources/odoo-live not found" in lowered or "bind-mount the project _resources" in lowered
...
def _check_host_source_packet() -> None:
    ...
    if missing:
        raise SourceDataUnavailable(
            "Host Odoo source packet is missing required file(s): "
...
def _check_container_source_packet() -> None:
    ...
    raise SourceDataUnavailable(
        "Container-visible Odoo source packet is missing or not mounted. "
        "This is a source-data/mount blocker, not a catalog price logic failure. "
        f"{detail} Run `python scripts/setup/stage_seed_data.py` or mount `_resources/odoo-live` into the backend container, then rerun."
    )
...
def bench_execute(method: str, *, kwargs: dict[str, Any] | None = None, timeout: int = 900) -> Any:
    ...
    if proc.returncode != 0:
        ...
        if _missing_source_hint(combined):
            raise SourceDataUnavailable(
                "ERPNext price modifier method could not see `_resources/odoo-live` inside the container. "
                "This is a source-data/mount blocker, not a catalog price logic failure.\n"
                f"STDOUT:\n{proc.stdout}\nSTDERR:\n{proc.stderr}"
            )
        raise ContractFail(
