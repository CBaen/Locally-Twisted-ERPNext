#!/usr/bin/env python3
"""Verify fake outbound document send-readiness scenarios.

Run:
  python scripts/verify/outbound_document_send_readiness_contract.py
  python scripts/verify/outbound_document_send_readiness_contract.py --json
"""
...
SITE = "frontend"
METHOD = "locally_twisted.verify.outbound_document_send_readiness_contract.run"


class OutboundDocumentSendReadinessContractFail(Exception):
    pass
...
def bench_execute() -> dict[str, Any]:
    ...
    if proc.returncode != 0:
        raise OutboundDocumentSendReadinessContractFail(
            f"bench execute failed\nSTDOUT:\n{proc.stdout}\nSTDERR:\n{proc.stderr}"
        )
    text = proc.stdout.strip()
    if not text:
        raise OutboundDocumentSendReadinessContractFail("send-readiness contract returned no output")
    try:
        parsed = json.loads(text)
    except json.JSONDecodeError as exc:
        raise OutboundDocumentSendReadinessContractFail(
            f"send-readiness contract returned non-JSON output: {text}"
        ) from exc
    if not isinstance(parsed, dict):
        raise OutboundDocumentSendReadinessContractFail(
            f"send-readiness contract returned {type(parsed).__name__}, expected object"
        )
    return parsed
...
def main() -> int:
    ...
    except OutboundDocumentSendReadinessContractFail as exc:
        print(f"[OUTBOUND DOCUMENT SEND READINESS CONTRACT] FAIL\n  - {exc}")
        return 1
...
def _contract_failures(result: dict[str, Any]) -> list[str]:
    ...
    if result.get("send_allowed") is not False:
        failures.append("contract itself allows sending")
...
def _print_summary(result: dict[str, Any], contract_failures: list[str]) -> None:
    print(
        "[OUTBOUND DOCUMENT SEND READINESS CONTRACT] "
