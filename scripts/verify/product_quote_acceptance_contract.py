#!/usr/bin/env python3
"""Verify accepted product-page quote to draft Sales Order behavior.
...
def main() -> int:
    ...
    try:
        result = bench_execute("locally_twisted.verify.product_quote_acceptance_contract.run")
    except ContractFail as exc:
        print(f"[PRODUCT QUOTE ACCEPTANCE CONTRACT] FAIL\n  - {exc}")
        return 1

    if not result or not result.get("ok"):
        print("[PRODUCT QUOTE ACCEPTANCE CONTRACT] FAIL")
        for failure in (result or {}).get("failures") or ["missing result"]:
            print(f"  - {failure}")
        return 1

    print("[PRODUCT QUOTE ACCEPTANCE CONTRACT] PASS")
