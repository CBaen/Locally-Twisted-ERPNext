#!/usr/bin/env python3
"""Verify the reusable LT product-page runtime contract.
...
def main() -> int:
    ...
    try:
        result = bench_execute("locally_twisted.verify.product_page_runtime_contract.run")
    except ContractFail as exc:
        print(f"[PRODUCT PAGE RUNTIME CONTRACT] FAIL\n  - {exc}")
        return 1

    if not result or not result.get("ok"):
        print("[PRODUCT PAGE RUNTIME CONTRACT] FAIL")
        for failure in (result or {}).get("failures") or ["missing result"]:
            print(f"  - {failure}")
        return 1

    print("[PRODUCT PAGE RUNTIME CONTRACT] PASS")
