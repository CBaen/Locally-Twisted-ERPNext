...
the current ERPNext/Frappe build already does well, and where an architectural
rebuild must pause if it would damage or mutate existing products.
...
## What LT Already Does Well

The current LT frontend has a strong balloon color-selection pattern. It
...
## What LT Still Needs Structurally

Product Setup still needs a stronger staff product console with these product
tabs and surfaces:
...
For the reviewed `Classic Organic columns` product, the target option model is:
...
LT also needs first-class related product structures:
...
ERPNext/Frappe instead of copying tables from any external reference.
...
by itself give LT the full distinction between SKU-defining attributes,
...
layer is the right place for that distinction.
