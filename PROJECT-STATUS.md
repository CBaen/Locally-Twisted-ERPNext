...
- **Current header/nav** uses the deliberate premium two-level mega-menu: `Event Balloons` and `Ready-to-Order` mega panels, `Portfolio`, `Process`, `FAQ`, search/cart, top proof row, full-height logo image treatment, and mobile drawer accordions. `Free Event Quote` points at `/contact`. Menu data lives in `navbar_context.py`; shop/sidebar defaults live in `website_context.py`; both are active website context hooks.
...
- **Navigation/routing cleanup superseded by the 2026-05-05 premium mega-menu:** primary desktop IA is `Event Balloons`, `Portfolio`, `Process`, `Ready-to-Order`, `FAQ`, search/cart, with `Free Event Quote` on `/contact`. `/event-balloons`, `/portfolio`, `/process`, `/shop`, `/faq`, and `/contact` return 200 locally. `/book` redirects to `/contact?intent=quick`; `/shop-items`, `/all-products`, and `/shop-by-category` send broad browse traffic to `/shop`.
...
- **Layout-fit gate current 2026-05-05:** `scripts/verify/layout_fit.spec.js` checks 20 public/shop/cart/checkout/success routes across 320px, 375px, tablet, and desktop viewports. Latest verified command: `npm run test:layout-fit` -> 80 passed.
