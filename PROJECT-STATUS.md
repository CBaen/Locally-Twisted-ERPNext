...
> **Public ecommerce testing correction, 2026-05-17:** Local ecommerce may be temporarily opened for proof runs, but the safe resting state is `lt_ecommerce_paused=1`. The current local product-import proof includes 53/53 Odoo-imported products as sellable checkout targets, 0 exclusions, and 290 priced sale units. This does not approve staging/live cutover; GL local testing, staging proof, live payment readiness, and DNS/Frappe Cloud gates still stand.
...
> **Staging release failure forensic correction, 2026-05-23:** The Frappe
> Cloud owner-review staging attempt failed as a release process. Do not treat
> any commits, app mirror hashes, deploy IDs, or interrupted bootstrap attempts
> from that session as owner-review readiness or launch authority. Use
...
- **Verified DB counts rechecked 2026-05-18: 53 Website Items, 10,674 Items total, 49 variant templates, 8 non-variant root Items, 10,227 active customer-facing variants, 390 disabled legacy optional-add-on variants, 10,617 all variant records, 10,656 Item Prices, 32,028 Item Variant Attribute child rows, 29 Item Attributes.** The original 10,578 count is now historical baseline language, not the active customer-facing contract.
...
- **Current header/nav** uses the deliberate premium two-level mega-menu: `Event Balloons` and `Ready-to-Order` mega panels, `Portfolio`, `Twisting & Face Painting`, `About Us`, `FAQ`, search/cart, top short-notice banner, full-height logo image treatment, and mobile drawer accordions. The desktop top banner keeps `Free Event Quote` top-banner-only and links the short-notice message to `/contact`; the menu/drawer CTA is `Contact Us`. `/process` is not a public launch route unless GL reopens it. Menu data lives in `navbar_context.py`; shop/sidebar defaults live in `website_context.py`; both are active website context hooks.
