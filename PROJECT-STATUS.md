...
- **Current header/nav** uses the deliberate premium two-level mega-menu: `Event Balloons` and `Ready-to-Order` mega panels, `Portfolio`, `Twisting & Face Painting`, `FAQ`, search/cart, top proof row, full-height logo image treatment, and mobile drawer accordions. `Free Event Quote` points at `/contact`; `/process` is not a public launch route unless GL reopens it. Menu data lives in `navbar_context.py`; shop/sidebar defaults live in `website_context.py`; both are active website context hooks.
