...
archives of retired platform/host history. Old labels, placeholder aliases,
local mirror/snapshot folders, retrospective packets, and old-host references
are forbidden in active tracked guidance. External backups and git history are
the archive.

**Reasoning:** GL identified that stale references were poisoning future-agent
source-of-truth selection. Renaming the retired implementation with a placeholder is still
preserving the stale source; it causes future agents to plan from the wrong
mental model.

**Implementation boundary:** Keep current neutral catalog-data/import
identifiers only where they are active implementation fields or verifier/report
handles. Do not use them as permission to restore old-platform context into
docs. If GL asks for external recovery, perform a bounded recovery task and
translate only the current result back into this repo.

**Verification:** 2026-07-02 cleanup passed text scans, filename scans,
old-host/reference scans, generic old-platform phrasing scans, and
`git diff --check`.

**Decided by:** GL directive; implemented by Codex with real multi-agent triad
...
source sync now dedupes catalog_data paired images, fills Product Setup gallery rows,
...
set using catalog_data option price modifiers; the reported product now resolves
...
current import capture/catalog_data `` is old reference/decommission scope, not the
...
**Decided by:** GL request to confirm Cloudflare/Frappe Cloud/current import capture state and
...
not all have the same checkout risk. "Every imported catalog_data item is a product"
...
imported catalog_data product is a product. The safer technical response is not to keep
...
**Receipts:** `workstreams/ecommerce-audit/catalog-data-sellable-product-reimport-2026-05-17.md`;
...
that differ only by casing, and the catalog_data value-normalize map already collapses
...
catalog_data export/value-normalize map or by an approved supplier/color source.
...
Pricing and product details come from the catalog_data product export list; if a
...
existing catalog_data products. catalog_data is a useful behavior witness, but the actual
...
For complex catalog_data-equivalent products, build backend-authored UI contracts:
...
that changes launch/business behavior. If a question crosses into catalog_data CE 19
source mapping beyond approved read-only packets, route it to the
catalog_data-to-Frappe specialist instead of guessing.
...
**Decision:** The catalog_data-to-ERPNext ecommerce audit may only cite lanes that produced named artifacts under `workstreams/ecommerce-audit/`, or lanes explicitly marked `[NO EVIDENCE]`. Lane F synthesis must wait until missing lanes A/E are rerun artifact-first or intentionally carried as process failures.
...
## 2026-05-10 - catalog_data is a source witness, not implementation source

**Decision:** Ecommerce migration work may use catalog_data-derived surfaces and module behavior as business/source evidence, but receiving functionality must be native ERPNext/Frappe: DocTypes, custom fields, app services, templates, browser contracts, and backend verifiers owned in `locally_twisted`. Do not copy catalog_data code or treat catalog_data module parity as the launch architecture.

**Reasoning:** The business needs catalog_data-derived product meaning preserved, not an catalog_data implementation transplanted into ERPNext. Copying code would import incompatible assumptions and make the destination harder to verify. The safe pattern is source meaning in, native contract layer out.
...
**Alternatives considered:** Directly port catalog_data module logic. Rejected as unsafe and outside the destination architecture. Delete/reimport products before proving the receiver. Rejected because it risks losing customer/operator intent.
...
## 2026-05-08 - catalog_data dynamic resolver is the price source for catalog variants

**Decision:** LT catalog variant prices must be imported, audited, and repaired
from catalog_data's dynamic `/website_sale/get_combination_info` resolver, not from the
...
Medium, and Large all at `$35`. catalog_data's resolver returned Small `$35`, Medium
`$70`, and Large `$85`. The original scraper and seed path flattened variant
prices because they copied the page base price into each variant row. Full catalog_data
...
**Implementation:** `scripts/setup/scrape_catalog_data_live.py` now resolves dynamic
prices through catalog_data's resolver. `seed_catalog.py` prefers
`erpnext_variant_price` for Item Prices. `repair_variant_prices_from_catalog_source.py`
...
assumption. Bulk-update every variant from catalog_data immediately. Rejected because
...
**Verification receipt:** Active app source search found no remaining `DM Serif`, `Raleway`, `Montserrat`, `Playfair`, `lt-blush`, `lt-soft-blue`, `soft blue`, `light blue`, `--lt-primary`, or UI `blush` references after the cleanup. The current non-catalog_data `_resources` search only finds explicit "deleted/do not use" notes in `_resources/STYLE-GUIDE.md`. SVG XML parse validation passed for all 16 brand icons. Python compile passed for `apps/locally_twisted/locally_twisted`, and LT CSS token usage had no missing `--lt-*` variables.
...
**Decision:** Translate the approved six-stage catalog_data CRM concept into ERPNext as `Lead.custom_pipeline_stage` with these values: `New Inquiry`, `Quote Sent/Awaiting Approval`, `Approved`, `In Production`, `Event/Post Event`, and `Archive`. Keep ERPNext's native `Lead.status` intact.

**Reasoning:** GL clarified that catalog_data's `Archive` was meant to remove a lead from the active Kanban, not necessarily to mark revenue, win rate, or accounting state. Repurposing ERPNext `Lead.status` would risk fighting ERPNext's own conversion/reporting behavior. A custom business-stage field gives Jeff the simple board he needs while leaving finance/reporting triggers to be wired deliberately at the correct business threshold later.
...
**Decision:** Product detail pages should switch the main image when the selected ERPNext variant has its own `Item.image`. The first media sync maps scraped catalog_data extra images onto variants only when the image URL/filename label clearly matches an option such as size, height, length, design, LED lights, topper, or theme. Generic or ambiguous gallery images remain unmapped until GL/Jeff review.

**Reasoning:** GL clarified that multiple photos usually belong to the size/variant option and that balloon decor images must respect the real product structure rather than random color/photo assignment. The catalog_data scrape includes many extra images, but not all filenames describe a variant. A conservative mapper preserves customer trust: use specific evidence, fall back to the parent product image when uncertain, and leave the rest for human/business review.
...
## 2026-05-02 - catalog data evidence is historical business input, not an app build target

**Kubuntu correction:** the old external-backup catalog-data project is
historical business evidence only when deliberately restored from backup or
quoted in retained source docs. Current business truth for this repo comes from
approved `_resources/` material, current decisions, verified app/database state
where it can prove the claim, and fresh GL/legal approval. The ERPNext repo
remains the app build target for launch.

**Reasoning:** The old catalog-data project captured business discovery,
catalog detail, policy detail, voice, and historical business context, but it is
not an active local working dependency on Kubuntu. ERPNext copy remains suspect
unless it can be traced to retained current `_resources/` material, current
decisions, or GL/legal approval. Keeping business truth separate from app builds
prevents agents from treating accidental ERPNext copy as authoritative.

**Operational impact:** Agents should not depend on a local catalog-data
checkout being present. When customer-facing copy, policies, product/service
claims, or business positioning matter, use retained current sources first and
ask GL before choosing product/business truth from stale artifacts. Verify
implementation behavior in ERPNext separately; historical evidence is not
current app-state truth.
...
**Alternatives considered:** Keep the current import capture mirror's historical menu labels as placeholders for future Phase 2 pages. Rejected because current navigation must only expose real or intentionally active routes.
...
**Reasoning:** Lower blast radius. Restructuring the catalog tree would risk the just-verified data integrity (53/10,578/10,613). Template-level grouping is reversible — if GL prefers the 2-level Item Group tree structure later, we restructure `fixtures/item_group.json` and re-tag the 53 Website Items, and the navbar template adjusts. The "flat + template-group" choice preserves all existing investment in catalog data while delivering current import capture's 3-panel UX.
...
**Decided by:** Claude Opus 4.7 (orchestrator), under GL's autonomous-decision authorization for the chrome rebuild session. **Reversible** — retained here as Decision A after the local mirror rebuild plan was retired from the Kubuntu checkout.

---

## 2026-04-30 (evening) — Category URL shape: ERPNext-native `/shop-items/<slug>` retained, NOT current import capture's `/shop/category/<slug>-<id>`

**Decision:** ERPNext webshop's `WebshopItemGroup.make_route()` auto-generates `/shop-items/<slug>` from the Item Group's `route` field (no catalog_data-style numeric IDs). The current import capture mirror uses `/shop/category/<slug>-<id>` URLs. Rather than mimic current import capture's URLs exactly, we use ERPNext-native `/shop-items/<slug>` everywhere. Mega menu links + footer Shop column links + breadcrumbs all use the ERPNext shape.

To handle inbound references to current import capture-shaped URLs (none exist externally today, but mirror markup contains them and Phase 2 page rebuilds may reference them):  add `website_route_rules` redirects from `/shop/category/<slug>` → `/shop-items/<slug>` for the 11 known categories when a real referrer surfaces.

**Reasoning:** Doesn't fight ERPNext's `make_route()` convention. No need to manually set `route` on each Item Group (which would be operator-state-sensitive — Jeff might rename a category later and the URL would break). Lower blast than redirect rules per category. Pre-launch — no external bookmarks to preserve. current import capture-shape URL preservation can be added later via redirects if real inbound traffic appears.
...
**Reasoning:** GL directive 2026-04-30: *"it is a migration, not a new build."* The 2026-04-26 reframe was motivated by (a) Jeff-disclosure concerns — the failed catalog_data attempt is BBC-internal context Jeff hasn't been briefed on yet — and (b) avoiding a too-mechanical "translate catalog_data → ERPNext" mental model. Both concerns remain valid, but neither justifies denying the technical reality:

- Catalog data was ported from the catalog_data deployment to ERPNext on 2026-04-30 (53 Website Items, 10,631 Items, 10,578 variants, 10,613 Item Prices — verified against the running DB).
- Form intent is owned by the current ERPNext `/contact` inquiry flow and its
  committed verifier coverage.
- Business policies, brand identity, voice rules, and the legal interview answers all originated in the catalog_data phase and were brought across into `_resources/`.
- At cutover (Phase 6), the new ERPNext storefront replaces `locallytwisted.com` at the same domain.

The right framing: **migration of business intent + catalog data into a fresh ERPNext install.** "Fresh install" captures that we did NOT auto-translate catalog_data modules / DB dumps / configuration — the destination was greenfield ERPNext, hand-built informed by catalog_data discovery. "Migration" captures the truth about catalog records, form schema, policies, and the eventual domain cutover.

**What stays from the 2026-04-26 reframe:**

- **Jeff-disclosure stealth.** Jeff knows there's an audit; he doesn't know the catalog_data attempt failed in testing. Internal docs use migration framing; Jeff-facing communications still don't leak that context until Phase 1 is demo-ready.
- **Hand-build, not auto-translate.** No automated catalog_data-to-ERPNext module/data conversion tooling. Catalog data was the only record-level port; everything else was hand-built from discovery.
- **`_resources/` is canonical and platform-agnostic** in language. Anything from the catalog_data dir that applies has been copied + scrubbed.
- **Reference Disposition stands.** `external-catalog-data/` clone, `` current import capture deployment, `CBaen/external-catalog-data` GitHub repo, and current `locallytwisted.com` all retire at/post-cutover.
...
## 2026-04-30 — Catalog rebuild from then-live catalog data (historical)

**Historical decision:** the then-live catalog data at
`shop` was the source for the 2026-04-30 catalog rebuild.
The current retained catalog source artifact lives at
`_resources/catalog-source/catalog.json`. The old live scrape script is not an
active Kubuntu tool; before any new catalog work, use current approval trails
and current verifiers rather than reintroducing the old source dependency.

**Reasoning:** GL directive 2026-04-30: *"the only source of truth is the live site... pull every product from the live site... with every single variation that a product has."* The cached catalog had 51 products; the live re-scrape on 2026-04-30 found 53 (catalog_data had added `birthday-deliveries`, `large-head-missionary` since the cache). 5 products had `image_url=null` cached but DO have images on the live site (the original scraper missed lazy-loaded `data-src` patterns). Also catalog_data's per-product `data-attribute-exclusions` JSON was not captured in the original scraper — the new live scrape captures and respects it.
...
**Decision:** Every catalog_data-valid attribute combination becomes an `Item Variant` record. Honoring catalog_data's `data-attribute-exclusions` to filter forbidden combinations from the cartesian product. Verified DB counts on 2026-04-30: 53 Website Items, 10,631 Items total, 49 variant templates, 4 single-SKU templates, 10,578 variants, 10,613 Item Prices, and 32,002 Item Variant Attribute child rows.

**Reasoning:** GL directive 2026-04-30: *"ALL VARIANTS DO NOT SKIP ANY."* Earlier in the same session I had proposed a "form-fed options" alternative (treat each product as single Item, render LT-owned color/size selectors at order time) — GL named it as me trying to "divert" from the task. Rebuild catalog_data accurately means full ERPNext variant model.
...
**Reasoning:** GL directive: mega menu populated from catalog_data's natural taxonomy. The taxonomy is implied by catalog_data product slug patterns (`*-arch`, `*-column`, `*-bouquet`, etc.) — formalized as 11 explicit BBC-decision Item Groups. Captured as a fixture so it's reproducible on transfer.
...
**Decided by:** GL 2026-04-29, after seeing the side-by-side comparison (catalog_data's flow vs. our Frappe form vs. Stripe's hosted page).
...
Full rationale: `.planning/decisions/site-shape.md`. Cover story for Jeff: *"We couldn't use catalog_data, so we had to rebuild on a different program. While I was rebuilding, I looked at how every other custom-balloon and event-decor company in our tier is structured today — Partistry, Balloon Emporium, the wedding florists. None of them sell custom installs through a checkout flow."*
...
2. **Slice 2 chrome (header + footer)** — Jinja partial overrides at `templates/includes/{navbar,footer}/`, replaced Frappe's defaults with the approved catalog_data two-tier desktop / single-row mobile structure. GL iterated on logo size, footer centering, footer padding, and 3-column-on-mobile spec; technique held under those iterations. GL confirmed: *"so far so good! It's getting better."*
...
## 2026-04-26 (session end) — Approved Jeff content is NEVER invented
...
1. **Retained catalog-data evidence captured in repo research** — use
   `research/expedition-frappe-theme/ground-truth-findings.md` and current
   `_resources/` material rather than assuming an external-backup checkout is
   available or authoritative.
...
- Before writing any text that will appear on a customer-facing page, READ the catalog_data XML or scrape the live site and use the actual content.
...
- Hero copy: "Utah's Balloon Specialists" / "Making celebrations unforgettable since 1998" (catalog_data) vs "Make Your Party POP!" / "Anything you imagine, we can shape into reality" (live site)
- Social icons: 3 (Facebook, Instagram, Pinterest — catalog_data) vs 4 (+ Twitter — live site)
- Credentials: "since 1998" / 28 years (catalog_data) vs "Over 22 years" (live site)
- Tagline: "Utah's Balloon Specialists since 1998." (catalog_data) vs different framings on live site
...
**Decision:** All Built by Cameron client builds default to ERPNext's native HRMS / Payroll module. Gusto is removed from the LT ERPNext-side project scope: no Gusto credential fields, no `gusto_service` Python helper, no Gusto CSV export job. The Gusto integration in the failed catalog_data attempt was **never wired or used** (per GL clarification 2026-04-26) — the catalog_data files are dead code on a never-launched test deployment.
...
**Decision:** When the Phase 2 translation reaches `twilio_service.py`, do NOT create a new DocType for it. It was `models.AbstractModel` in catalog_data (no records, only methods bound to a model namespace for `env["..."].method()` invocation). The Frappe-equivalent is Python helper functions inside a custom Frappe app, OR Server Scripts bound to a hook — not a DocType.
...
**Decision:** All translation work (catalog_data → ERPNext) happens against the local LT install at `:8081`. No custom Frappe app scaffolding, no bench planning, no Frappe Cloud setup, no transfer-to-Jeff machinery until there is something real to transfer.
...
**Decision:** Phase 1 (Inventory, INV-01 + INV-02) plans exist on disk but will NOT be executed. The off-catalog_data expedition's `external-catalog-data/research/extended-expedition-off-catalog_data-replacement/inventory-findings.md` is treated as the working inventory baseline. INV-02 (production arch_db read) is deferred to a late phase — content migration concern, not rebuild concern.
...
## 2026-04-25 evening — Don't modify old catalog-data sources

**Decision:** All scripts, tools, and code written in service of the migration go in `_CLIENTS/locally-twisted/`. External backup material is historical read-only evidence only when GL explicitly restores it for a bounded task. Even "operational" tooling from that material is off-limits.

**Reasoning:** GL: "leave catalog_data specific scripts and skills alone. we need to create ERPNext specific ones." ERPNext-side tools are separate concerns and stay separate.

**Alternatives considered:** Modify `deploy.py` to add an `--inventory` subcommand (rejected by GL for the rule above). Use catalog_data's MCP server (currently disconnected, status uncertain).
...
**Reasoning:** Standard with Numbers matches catalog_data's default convention (carryover for Jeff's familiarity). Calendar year is US small-business default; no indication LT has a different fiscal year. Services is the closest fit for event services (balloon decor, twisting, face painting); Retail is less natural (LT is mostly service work, not goods sale).
...
**Decision:** Do not treat catalog_data-derived product transfer as the current goal. Real product import is blocked until ERPNext/Frappe has a designed, verified ecommerce receiving architecture that can carry product meaning everywhere: backend fields, product type/template classification, variants, add-ons, cascading dependencies, dynamic pricing, product detail rendering, media/variant visibility where source provides it, cart, checkout, Sales Order, invoice, fulfillment/operator meaning, mobile/desktop customer journeys, and fail-loud reports.

**Reasoning:** ERPNext native ecommerce is insufficient visually, logically, and operationally for Locally Twisted. catalog_data ecommerce is not the infrastructure to copy, but it is a conceptual witness for mature ecommerce behavior. Copying catalog_data field names or data into ERPNext without ERPNext-side logic creates silent failure: fields do not exist, frontend sections do not populate, price/media/option logic drifts, and invoices lose purchase meaning.
...
**Clarification:** Missing variant images are not automatically a defect. Only flag a missing variant/media mapping when the catalog_data/source data says that variant has or should expose a specific image. Many variants may legitimately share product media.

**Next required step:** Draft a research brief using the Claude `research-brief` skill shape, then run `/expedition` only after GL approves the brief. Research must cover both ERPNext/Frappe implementation patterns and catalog_data ecommerce concepts/behaviors that should be recreated safely inside ERPNext.
