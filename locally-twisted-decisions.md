...
**Status:** Updated by the same-day crawl regression follow-up. The stable hero
rules remain current; the proof crawls now move right-to-left at matched visible
...
**Implementation:** Updated `www/home.html`, `www/home.py`, `lt-theme.css`, `lt-page-containment.css`, `lt-site-preferences.js`, and homepage-focused Playwright coverage. The first repair removed the hidden/rotating H1 pattern, promoted Recent Celebrations before reviews, used approved brand icon masks in the authority band, and moved cookie consent into the document flow on the homepage. The follow-up repair changed both crawls to right-to-left, added a homepage-only measurement script that syncs the trusted-business crawl to review-card pixel speed, and changed the reduced-motion branch/test contract away from static scrollbar fallback.

**Verification receipt:** `python scripts/dev/clear_website_cache.py` passed. The backend container was restarted after controller CSS changes so `PAGE_CSS` was re-imported. Homepage internal links checked 37 links with no failures. `npx playwright test scripts/verify/layout_fit.spec.js --reporter=dot --grep "home fits"` passed 13/13. `npm run test:interactive-layout -- --grep "homepage|cookie notice"` passed 11/11. Screenshots were captured under `output/playwright/landing-fixes-20260507/`. Follow-up full-site closeout passed `npm run test:website-verify`: nav IA, `layout-fit` 247/247, `interactive-layout` 74/74, checkout 2/2, portfolio reel 4/4, and shop smoke. Same-day crawl regression follow-up passed `python -m py_compile apps\locally_twisted\locally_twisted\www\home.py`, `node --check scripts\verify\interactive_layout.spec.js`, `python scripts/dev/clear_website_cache.py --restart`, focused crawl regression 5/5, home layout 13/13, homepage/cookie 12/12, and compact hero 14/14. Live Playwright diagnostics showed both crawls moving right-to-left with hidden overflow and near-zero speed delta in `no-preference` and `reduce`. Full `npm run test:website-verify` then passed after the crawl repair: nav IA, `layout-fit` 247/247, `interactive-layout` 88/88, checkout 2/2, portfolio reel 4/4, and shop smoke.
