...
Phase 5 added source-level Product Setup operating-brand authority controls:
`LT Product Blueprint` now has a required `operating_brand` Select field with
allowed lanes `locally_twisted`, `commercial_balloon_decor`, and
...
Phase 11 added offline no-write replacement model tooling and ran it against
the saved Birthday Deliveries Phase 9/10 artifacts plus the saved source
artifact. It produces three design-only candidate SKU rows for `Small`,
`Medium`, and `Large`, moves `Delivery themes` to configuration payload, and
holds `Add Foil Number` plus `Add Bouquet` as paid add-on candidates. The
report intentionally exits `1` with 27 blockers, preserving the current 2,430
variant Items / 2,430 Item Prices and requiring add-on/runtime pricing,
payload-preservation, live route, brand-lane, historical-reference,
...
Phase 14 added a read-only Desk `Show Readiness` button for Product Setup. It
displays the saved validation readiness state, next owner step, public-success
permission, live apply permission, and blockers without adding any live write
...
Phase 15 added offline catalog readiness dashboard tooling over saved Phase 4
authority packet reports. Against the saved full-catalog packet it intentionally
exits `1` with 47 products, 47 blocked products, 284 blockers, six
variant-explosion products, zero ready products, and all local/staging/live
apply, mutation, cache clear, deploy, and public-success approvals false. It is
...
Phase 17 added a read-only Desk `Show Catalog Readiness` summary for Product
Setup. It reads saved `LT Product Blueprint.validation_json` rows only, shows
catalog counts, blocked rows, saved proof mode, saved evidence time, next owner
step, and whether developer help is needed. It does not call preview/apply
methods, mutate ERPNext records, clear cache, deploy, contact providers, touch
