...
- `/event-balloons`, `/portfolio`, and `/balloon-twisting-and-face-painting` are real public routes and return 200 locally. `/process` was unapproved and has been removed from the customer-facing site contract. `/portfolio` now keeps only the approved collage-of-imagery behavior from the external prototype: compact `What We Do` intro, native LT shell/global typography, large whole installed-work images, frequent center-column photos, optimized WebP derivatives, no cropped cards, visible captions below photos, and click-to-front interaction. Do not reintroduce the tall prototype hero, portfolio-specific font imports, custom cursor, fake shell, or full Claude/designer page styling. Category/event query links still filter the photo payload server-side. The research folder remains critique input only; the Frappe implementation, optimized assets, and verifier are the kept production source.
...
- Homepage launch repair completed on 2026-05-07: the hero uses one visible stable H1, the first viewport shows the next band on desktop and 320px mobile, the cookie notice renders as an inline homepage band instead of covering CTAs, authority proof uses the approved brand icon masks, Recent Celebrations appears before review cards, the closing CTA leads with corporate/school/civic/community work, and stale homepage v2/design-studio comments were removed.
...
- `scripts/verify/layout_fit.spec.js` is the committed passive Playwright Test gate. Latest verified command: `npm run test:layout-fit` -> 247 passed across the current route list and 13 viewport families, including `/checkout` and `/thank-you`. `scripts/verify/interactive_layout.spec.js` is the stateful layout gate; latest verified command: `npm run test:interactive-layout` -> 74 passed. `scripts/verify/portfolio_reel.spec.js` is the route-specific proof-gallery gate; latest focused verified command: `npm run test:portfolio-reel` -> 4 passed.
