...
Last updated: 2026-05-07 by Codex after committing the remaining Event Playground/OpenClaw research handoff, implementing the compact hero contract, clarifying homepage proof order, removing the homepage trust bar, repairing the portfolio shell around the approved collage/movement, restoring Twisting & Face Painting to the primary public navigation, removing the unapproved Process page from customer-facing chrome, and preserving the existing paperwork/backend, category-media, checkout, responsive/public verification, and launch-readiness notes.
...
- Homepage review cards and the trusted-business client crawl both crawl left-to-right as full-stage horizontal proof lines in normal motion. Both use `540s` animation duration; reduced-motion mode keeps both proof bands horizontal/full-stage and static so they do not stack or become broken scrollbars.
