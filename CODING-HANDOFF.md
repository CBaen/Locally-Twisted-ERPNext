...
- `/event-balloons`, `/portfolio`, and `/process` are real public routes and return 200 locally. `/event-balloons` and `/process` are lightweight authority pages added so the current primary nav has no dead links. `/portfolio` now uses a proof-first floating photo reel: natural-ratio installed-work images, optimized WebP derivatives, no cropped cards, no visible captions over photos, and click-to-front interaction. Category/event query links still filter the photo payload server-side. The Claude/designer reference folder remains in `research/` for active critique; the Frappe implementation, optimized assets, and verifier are the kept production source.
