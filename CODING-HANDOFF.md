...
commits and `git status` is clean. GitHub auth was repaired on 2026-06-16 with
`gh auth login` plus `gh auth setup-git`; `git push origin main` then advanced
`origin/main` to `af8a382`. This is source archive only, not staging/live
release approval.
...
the old current import capture/catalog_data IP `` is reference/decommission scope, not
...
The bouquet-size family was repaired from catalog_data's dynamic resolver and is
...
price even though catalog_data's dynamic resolver returned `$375` and `$440`. Treat this
...
The ERPNext build is active at `http://localhost:8081`. The project is **a migration of Locally Twisted's business intent + catalog data into a fresh ERPNext install** (frame revised 2026-04-30 — see `locally-twisted-decisions.md`). "Fresh install" — destination is greenfield ERPNext; no auto-translated catalog_data modules or DB dumps. "Migration" — catalog records (53 Website Items / 10,578 original variants / 10,613 original catalog Item Prices, ported 2026-04-30), form intent, policies, voice/brand all carried across from the catalog_data attempt and the legacy `locallytwisted.com` site, and the new storefront replaces `locallytwisted.com` at cutover. Current local DB counts are listed below; older count language is historical unless rechecked against the target DB.

The catalog port from the catalog_data test deployment appears real, but several docs had stale counts. The catalog_data shop at `shop` was used as the catalog source/reference for that port because GL explicitly named it as the old live account/source for catalog data. That does not make catalog_data the product truth for unrelated business scope.
...
- Variant media first pass completed 2026-05-02. ERPNext now has 1,712 variant `Item.image` values mapped from `_resources/catalog-source/images/` where catalog_data image labels clearly matched product options. Product detail pages call `locally_twisted.api.variant_media.get_variant_media` after exact option selection and swap the main image when a variant image exists. Cart/checkout use the variant image when present and fall back to the parent Website Item image otherwise. The review command `python scripts/setup/sync_variant_media.py --dry-run --include-details --report output/catalog-media-review.json` currently reports 49 products checked, 35 with candidate image labels, 45 needing review, 1,712 unchanged mapped variants, and 6,831 skipped variant image assignments.
...
- Catalog variant counts match the normalized catalog_data source: the raw scrape has duplicate-case latex color values, but `_resources/catalog-source/value_normalize_map.json` collapses them and the normalized expected variant counts match ERPNext.
...
- Existing docs say `24` Item Attributes from the catalog_data-derived catalog, but the DB currently has `26` Item Attribute records. Investigate before changing fixture logic.
...
docker exec locally-twisted-erpnext-v15-backend-1 bench --site frontend execute locally_twisted.seed.repair_variant_prices_from_catalog_source.execute --kwargs "{'slug_filter':'unicorn-bouquet','dry_run':True}"
```
...
Variant media sync from the captured catalog_data image files:
