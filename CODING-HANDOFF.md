...
Codex update on 2026-05-10: header/banner cleanup is now documented and
...
gold `/contact` strip on desktop and mobile, and `Ready-to-Order`, `Cart`, and
`Recent Work` are forbidden from the top banner. The follow-up accessibility
review fixed low-contrast hover/focus text and reversed mobile safe-area
padding. Feature handoff:
...
Last updated: 2026-05-10 by Codex after public ecommerce reopened for full local testing, ecommerce verifiers were moved out of pause-only mode, and checkout/backend cascade gates were re-run. Current peer handoff: fake data is allowed and useful; fake success is forbidden. Public ecommerce is in local full-testing mode, not production cutover mode.

Codex update on 2026-05-10: GL reopened public ecommerce for full local testing.
The local site is currently configured with `lt_ecommerce_paused=0`; do not
treat pause-mode results as sufficient proof. `npm run test:ecommerce-full`
passed the rendered and rollback-safe ecommerce path: public ecommerce mode,
shop smoke, product prices, variant media, checkout experience, checkout
fulfillment, and checkout-to-Lead conversion. `npm run test:public-verify` now
includes the open ecommerce mode contract, shop smoke, product prices, variant
media, and checkout experience. `python scripts/verify/product_page_architecture_readiness.py`
also passes with `technical_architecture_ok: True`, `import_reopen_ok: True`,
14 passing rows, 0 blockers, and 1 finance deferral. This is a local testing
state, not live production approval; staged client review, live payment/DNS
cutover, and the agency preflight/staging-to-live process still apply.
...
- Header/menu uses the deliberate premium two-level mega-menu: full-height Locally Twisted logo image, desktop `Event Balloons`, `Twisting & Face Painting`, `Ready-to-Order`, `Portfolio`, `About Us`, `FAQ`, top utility row, search overlay, cart, and `Contact Us` CTA when `lt_ecommerce_paused=0`. `Twisting & Face Painting` points to `/balloon-twisting-and-face-painting`; `Ready-to-Order` points to `/shop`; `Contact Us` points to `/contact`; top-banner `Free Event Quote` points to `/contact` and the account link remains. The short-notice copy `SHORT NOTICE? LET US KNOW. WE CAN OFTEN HELP WITH 24 HOURS NOTICE!` is a centered gold `/contact` link on desktop and a matching visible gold `/contact` strip on mobile; the old `Prepared design, clean installs, and invoiced event support across Utah.` proof copy and delivery/truck icon are removed. Ready-to-Order and cart remain config-gated for future pause mode, but the current local testing state is open. Menu coordination lives at `workstreams/menu-content-coordination.md`; the active service-removal guard handoff is `workstreams/nav-service-removal-guard.md`; the older BTFP/Process correction handoff remains historical context at `workstreams/nav-btfp-process-correction.md`.
- Mobile header compactness is now part of the nav contract: in open ecommerce testing the header row carries logo, cart, and menu. Search lives as a bottom drawer button, opens the overlay, closes the drawer first, and submits to `/shop`; `/search` is kept as a no-cache 404 fallback, not a public page.
- Header color repair completed 2026-05-06 after GL flagged the all-black chrome as off-style and was updated 2026-05-10 for the short-notice banner. The mega-menu contract stayed intact, but `lt-mega-menu.css` now uses a style-guide split: gold desktop/mobile short-notice strips, warm-white desktop main nav, warm-white mobile header/drawer surfaces, berry CTA, brass borders, and ink text. `hooks.py` cache-bust is `lt-mega-menu.css?v=20260510-short-notice-4`, and `interactive_layout.spec.js` guards against regressing the header shell back to the black ink band or the short-notice strips away from gold.
...
- Footer no longer exposes `What We Make` or `Book an Event`; `All Ready-to-Order` is config-gated and visible in the current open ecommerce testing mode, and `Twisting & Face Painting` remains visible as a service lane.
...
- `scripts/verify/layout_fit.spec.js` is the committed passive Playwright Test gate. Latest full launch run: `python scripts\verify\website_launch_verify.py` -> 12/12 website steps passed, including `layout-fit` 325/325 across the current route list and 13 viewport families, `container-contract` 75/75, `interactive-layout` 163/163, `search_contract` 3/3, `portfolio_reel` 6/6, open ecommerce mode, shop smoke, product prices, variant media, and checkout experience 2/2. `scripts/verify/container_contract.spec.js` is the route-level public container contract; focused rerun after the homepage Custom Event Decor repair passed 75/75. `scripts/verify/portfolio_reel.spec.js` is the route-specific proof-gallery gate.
