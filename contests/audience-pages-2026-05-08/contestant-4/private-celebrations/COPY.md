# /private-celebrations — Copy Sheet

## Hero

**Eyebrow:** Private Celebrations — Wasatch Front

**H1:** Custom balloon decor for the moments that matter most.

**Lede:** Birthdays, weddings, baby showers, milestone celebrations, and celebrations of life. Each one designed with care for the moment it represents — delivered to your venue, installed, done.

**CTA:** Tell us what you're imagining →

---

## Memorial Proof Callout (pre-grid, deep navy band)

**Label:** Celebration of Life

**Quote:** "I needed a sports themed funeral stand. I told them what I needed, they captured my vision, delivered on time, very reasonable, and had many compliments. Very tasteful and meaningful."

**Attribution:** — Google review, celebration of life

---

## Celebration Types Section

**Eyebrow:** What we do for private celebrations.

**H2:** Every milestone has its own design.

**Body:** Locally Twisted has produced custom balloon decor for hundreds of private celebrations across the Wasatch Front — from first birthdays to elegant weddings to celebrations of life. Each one is built from your vision, not a template.

### Card 1 — Birthdays
**Label:** Birthdays
**Proof:** 300+ birthday installs
**Body:** From first birthdays to milestone decades, custom balloon decor sized for living rooms, backyards, and venue rental spaces.

### Card 2 — Weddings
**Label:** Weddings
**Proof:** Wasatch Front weddings
**Body:** Organic arches, ceremony backdrops, and reception decor in coordinated palettes. Latex-free options available for allergy-sensitive ceremonies.

### Card 3 — Baby Showers
**Label:** Baby Showers
**Proof:** Deliveries across the valley
**Body:** Balloon bouquets, organic garlands, and full decor packages delivered to your venue — or installed on-site for larger showers.

### Card 4 — Memorials
**Label:** Memorials
**Proof:** Trusted for celebrations of life
**Body:** Celebration-of-life installations that honor a person with thoughtfulness and care. Sports themes, color tributes, and custom builds available on short notice.

---

## Standalone Memorial Section

**Eyebrow:** Celebrations of Life

**H2:** If you're here for a celebration of life, you're in the right place.

**Body P1:** Jeff and his team have helped families honor people they love. A sports theme. A favorite color. A tribute to something the person cared about. Whatever the context, the install is approached with care — and it arrives on time.

**Body P2:** There's no standard package for this kind of event. Share what you're imagining when you reach out. You don't need to have it figured out before you call.

**CTA (soft):** Start a conversation →

---

## Proof Pillars Band

**Section label:** How Locally Twisted works for private events

### Pillars

**Milestone-Ready**
Designed for the moments that deserve to be remembered. Every detail considered.

**Design Driven**
Custom builds matched to your theme, color, and vision — not a catalog selection.

**Delivered & Installed**
Full delivery, setup, and optional strike so you can focus on being present.

**Personal Attention**
Jeff and his team work directly with you. No order-and-wait experience.

---

## Testimonials

**H2:** What families say.

**Testimonial 1:**
"I have told so many people about how much we loved the balloon creations at a friend's wedding. We were seriously blown away and my kids were delighted."
— Google review, wedding

**Testimonial 2:**
"Jeff was super nice and helpful, helped me figure out the perfect thing for our son's birthday. Prompt, accommodating, great communication and friendly!"
— Google review, birthday

**Testimonial 3:**
"Locally Twisted has done a phenomenal job on many occasions. They are now my go-to easy decorating plan for any of my family events. I don't have to do anything, my house is festive, and I get to enjoy it too!"
— Google review, repeat family client

**Testimonial 4:**
"Amazing balloon arrangements — we had so many compliments. Jeff was easy to work with and the setup was exactly what we had in mind. Will definitely use again for future events."
— Google review, milestone celebration

*(Note: The KJSCOTT memorial review moved to the pre-grid callout band where it lands before the buyer reaches the category grid.)*

---

## Service Note

**H2:** How private celebrations work with Locally Twisted.

**Body P1:** Start by telling us what you're imagining — the event type, date, color palette, and any themes or specific requests. Jeff and his team will work with you to figure out the right scope and send a quote. Most private celebrations are quoted within one to two business days.

**Body P2:** Delivery and setup are available for most private events. Latex-free options are available for allergy-sensitive celebrations. For events that need same-day service, call (801) 285-0860 directly.

---

## Closing CTA

**H2:** Tell us what you're imagining.

**Body:** Every private celebration starts with a conversation. A quote is free — and usually back within two business days.

**CTA:** Start a Private Celebration Quote →

**Secondary:** Questions? Call (801) 285-0860 or hi@locallytwisted.com
