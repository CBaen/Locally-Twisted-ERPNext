# Private Celebrations — All Copy (Contestant 7)

## Hero

**Eyebrow:** Private Celebrations · Wasatch Front, Utah

**H1:** The room should feel like it was made for this moment.

**Lede:** Weddings, birthdays, baby showers, milestone celebrations, and celebrations of life — Locally Twisted makes the decor match the occasion, not the other way around.

**CTA:** Tell us what you're imagining →

---

## Section 2 — Intro

**Label:** The private record

**H2:** Every detail matters. Especially in private.

**Body:** Private celebrations carry weight that corporate events don't. A birthday milestone only happens once. A wedding ceremony can't be redone. A memorial needs to feel exactly right. Locally Twisted takes that seriously — the palette, the scale, the timing, and the teardown are all planned around the moment, not around the vendor's convenience. And what happens at your private event stays private.

If you're here for a celebration of life, you're in the right place. The same care that goes into a wedding arch goes into a tribute arrangement — and the process is quiet, direct, and doesn't add to what you're already carrying.

---

## Section 3 — Memorial & Celebration of Life (standalone section)

**Label:** Memorial & Celebration of Life

**H2:** When the occasion asks for something different.

**Body:** Planning a celebration of life is already a lot to carry. The decor shouldn't be an additional burden. Locally Twisted works directly with families and memorial organizers — no layers, no pitch process. Share what you have in mind, and a quiet conversation starts from there. The work is done with care, and nothing is shared publicly without your permission.

**Pull quote (KJ, memorial client):**
"I reached out with a vision for a sports-themed funeral stand. They captured it, delivered on time, and the result was very tasteful and meaningful."
— KJ, memorial client / Celebration of Life

---

## Section 4 — Category proof (birthdays, weddings, baby showers)

**H2:** Every kind of celebration

**Sub:** Balloon decor tailored to what the moment calls for.

### Birthdays
**Icon:** balloon-pair
**Category label:** Birthdays
**Headline:** 300+ birthday installs
**Detail:** From first birthdays to milestone decades. Delivered, installed, and ready before the guests arrive.

### Weddings
**Icon:** organic-garland
**Category label:** Weddings
**Headline:** Weddings across the Wasatch Front
**Detail:** Ceremony arches, reception garlands, and organic balloon decor in palettes matched to your flowers and venue.

### Baby Showers
**Icon:** balloon-cluster
**Category label:** Baby Showers
**Headline:** Baby showers and gender reveals
**Detail:** Tasteful, photograph-ready decor for the moments that matter before the baby arrives.

---

## Section 5 — Install proof

**H2:** Installed work

**Sub:** A sample of private celebration installs across the Wasatch Front.

### Card 1: Wedding
**Occasion label:** Wedding
**H3:** Organic half-arch at a Wasatch Front ceremony
**Body:** Organic-style balloon arch in venue-matched palette. Installed before the ceremony, removed after reception.

### Card 2: Wedding
**Occasion label:** Wedding
**H3:** Floral-style half-arch at a wedding reception
**Body:** Flower-shaped balloon clusters in a soft white and blush palette. Ceremony-to-reception transition decor.

### Card 3: Celebration
**Occasion label:** Celebration
**H3:** Foil heart arch for a milestone celebration
**Body:** Custom foil heart arch in celebration colors. Ready before guests arrive, photographed beautifully.

---

## Section 5 — Testimonials

**H2:** What people say

**Sub:** From birthdays, memorials, and long-time family relationships.

### Testimonial 1 (Sara, longtime client)
"Jeff has been listed in my phone for 7-ish years as 'balloon guy' and has been my go-to for that long. They make every event I plan easier and extra special."
— Sara, longtime client / Family events

### Testimonial 2 (KJ, memorial client)
"I reached out with a vision for a sports-themed funeral stand. They captured it, delivered on time, and the result was very tasteful and meaningful."
— KJ, memorial client / Celebration of life

### Testimonial 3 (Sarah, birthday parent)
"Jeff was super nice and helpful, helped me figure out the perfect thing for our son's birthday. Prompt, accommodating, great communication and friendly."
— Sarah, birthday parent / Birthday

---

## Section 6 — Capability bar

**H2:** What you can count on

**Sub:** The operational promise for private celebration clients.

### Taste-Matched
**Body:** Palette, scale, and style matched to your venue, your flowers, and your sense of the moment.

### Ready Before Guests
**Body:** Installation is complete before your event begins. No scramble, no last-minute. You enjoy it, not manage it.

### Photograph-Ready
**Body:** Decor designed to look beautiful in photos. The moments you capture should have a backdrop worth keeping.

### Private by Default
**Body:** Private events stay private. Client names and photos are not shared without permission.

---

## Section 7 — Service note

**Label:** For families, couples, and celebration planners

**H2:** How the process works for private clients

**Body:** Start with what you're imagining — the event type, approximate date, and a sense of the palette or style. A quote is returned that includes materials, delivery, setup, and teardown. Delivery times are coordinated so the decor is in place before the first guest arrives. For celebrations of life and memorials, the team communicates directly and quietly — the process doesn't add to the weight of the occasion. Photography is yours to share or keep private; Locally Twisted does not post client event photos without permission.

---

## Section 8 — CTA

**H2:** Tell us what you're imagining.

**Body:** The occasion, the rough date, and a sense of what you're envisioning. A complete quote — with delivery and teardown — comes back quickly.

**CTA button:** Start a conversation →

**Note:** Or call directly: (801) 285-0860
