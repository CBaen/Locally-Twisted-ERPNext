...
- [frappe-cloud-api-payload-shape-drift](failures/frappe-cloud-api-payload-shape-drift.md) - prevents Frappe Cloud deploy/update request acceptance from being mistaken for payload correctness when nested app/site data is stringified or incomplete.
- [frappe-cloud-deploy-site-object-drift](failures/frappe-cloud-deploy-site-object-drift.md) - prevents typed JSON deploy/update payloads from passing with name-only site rows; use the current provider site object with `name`, `server`, `bench`, `skip_backups`, and `skip_failing_patches`.
...
- [frappe-cloud-permission-role-fixture-order-drift](failures/frappe-cloud-permission-role-fixture-order-drift.md) - prevents local-only custom roles from breaking Frappe Cloud migration when permission sync creates `Custom DocPerm` rows before ensuring linked Roles.
- [staging-proof-surface-conflation](failures/staging-proof-surface-conflation.md) - prevents local tests, GitHub pushes, app mirror hashes, deploy candidates, site migration, cache clear, staging route proof, and live approval from being treated as the same release state.
...
- [release-controller-churn-after-stop](failures/release-controller-churn-after-stop.md) - prevents Frappe Cloud release work from continuing after repeated provider/bootstrap failure or a GL stop signal until an executable release lock, circuit breaker, and artifact-owned plan exist.
- [artifactless-subagent-release-triad](failures/artifactless-subagent-release-triad.md) - prevents release triads from becoming advisory-only when witnesses do not own concrete artifacts, gates, or blocking evidence before mutation.
