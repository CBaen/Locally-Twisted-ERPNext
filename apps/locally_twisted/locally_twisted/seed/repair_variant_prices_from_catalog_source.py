"""Repair ERPNext variant Item Prices from the catalog source's dynamic combination prices.

Run in-process:
    python scripts/setup/stage_seed_data.py
    bench --site frontend execute locally_twisted.seed.repair_variant_prices_from_catalog_source.execute --kwargs '{"slug_filter":"unicorn-bouquet"}'
    bench --site frontend execute locally_twisted.seed.repair_variant_prices_from_catalog_source.execute --kwargs '{"dry_run": true}'

This exists because the catalog source's product page base price is not enough. Price-affecting
...
class CatalogSourceVariantPriceRepairError(RuntimeError):
    pass
...
def _find_resources() -> Path:
    ...
    for candidate in candidates:
        if (candidate / "catalog.json").exists():
            return candidate
    raise CatalogSourceVariantPriceRepairError(
        "catalog source packet not found. Run `python scripts/setup/stage_seed_data.py` "
        "or mount `_resources/catalog-source` into the backend container."
    )
...
def _csrf_token(html: str) -> str:
    ...
    if not match:
        raise CatalogSourceVariantPriceRepairError("catalog source product page did not expose csrf_token")
...
def _post_combination_info(
    ...
    *,
    ...
    if data.get("error"):
        raise CatalogSourceVariantPriceRepairError(f"catalog source combination price error: {data['error']}")
    result = data.get("result") or {}
    if not result.get("is_combination_possible"):
        raise CatalogSourceVariantPriceRepairError(f"catalog source says combination is not possible: {ptav_ids}")
    if result.get("price") is None:
        raise CatalogSourceVariantPriceRepairError(f"catalog source returned no price for combination: {ptav_ids}")
    return result
...
def _repair_product(
    ...
    *,
    ...
    # Keep one catalog source call per current ERPNext variant, not per dropped add-on row.
    ...
    for row in projected_rows.values():
        ...
        if not variant_code:
            raise CatalogSourceVariantPriceRepairError(f"ERPNext variant not found for {slug}: {row.get('combo')}")
...
def execute(
    ...
    if slug_filter and not products:
        raise CatalogSourceVariantPriceRepairError(f"No variant product found for slug_filter={slug_filter!r}")
