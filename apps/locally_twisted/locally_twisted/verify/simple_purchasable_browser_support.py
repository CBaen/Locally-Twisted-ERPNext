"""Local browser-proof helpers for the simple purchasable product tranche.

These helpers intentionally mutate the local ERPNext site only when called by
the CLI wrapper. They snapshot the current Website Item contracts, apply the
...
def _snapshot() -> dict[str, Any]:
    rows = frappe.get_all(
        "Website Item",
        filters={"item_code": ["in", list(SIMPLE_BROWSER_PRODUCTS)]},
        fields=[
            "name",
            "item_code",
            "web_item_name",
    ...
    missing = sorted(set(SIMPLE_BROWSER_PRODUCTS) - found)
    if missing:
        raise ContractFail(f"missing Website Items for simple browser proof: {missing}")
