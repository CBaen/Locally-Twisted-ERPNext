...
**How "regions" map to construction physics.** The tap-and-fill UX presents regions as visual zones on the arch illustration; behind the scenes, each region corresponds to a color slot within the arch's design type. A Swirl arch supports up to 4 colors (per construction physics — Swirl is a 4-cluster spiral pattern where up to 4 distinct colors fit the repeat cycle); a Layered arch supports up to 8; an Organic arch is palette-driven (any count). When the customer taps "Main" and "Accent," they are assigning catalog colors to slots 1 and 2. The tool distributes those colors across the actual balloon cluster sequence using the gcd-based repeat math — the customer doesn't pick which cluster gets which color, they pick the palette. This is the Section 5.3 Option (c) hybrid: region-tap UX with color cap visibly enforced. The picker greys out additional color slots once the design's cap is hit. This honesty about limits is not a UX tax — it's what prevents a customer from designing a Swirl arch in 7 colors that Jeff cannot build.
...
- Per-piece, per-region color data: **color name first, hex code second**. Example: `Arch / Main: Empowermint (~#A8E6CF) · Arch / Accent: Blush (~#F4DFD7) · Arch / Pop: Lime (~#BFFF5C)`
- Total piece count
- Free-text field: "Anything else?" (optional)

**Why color names lead.** The catalog has 53 named colors; no official hex values exist yet (Jeff will add Pantone/hex mappings later). "Empowermint" is a specific SKU. A hex value like #A8E6CF is an eyeball approximation used in the picker for visual display, not a sourced match. When Jeff calls his supplier, he orders by name — "I need Empowermint 11-inch" — not by hex. The design card foregrounds names as the supplier-actionable identifiers and notes hex values as approximate display aids (~). This is the inverse of what most design tools do (lead with hex, treat name as label), and it's the correct inversion for Jeff's actual workflow.

Jeff opens his CRM at 8 AM and sees a Lead with this payload already populated. He has supplier-call-ready information before he picks up the phone: catalog color names for his balloon order, approximate hex codes for visual matching with the customer, piece names to estimate labor. He doesn't need to ask "what colors were you thinking?" — that question is answered. His pitch starts from "tell me about your event" not "what did you have in mind."
...
This is where the slot-based composition pattern earns its place. After the customer colors and adds their first piece, the composition view has empty slots with placeholder outlines: a soft gray arch-shaped silhouette in "Column" position, a soft gray round shape in "Garland" position. These aren't recommendations — they're empty slots in the customer's own design.

**Level clarity: the Zeigarnik mechanic operates at the composition level, not inside a single shape.** Within a single shape, fill regions follow color cap math: a Swirl arch enforces up to 4 color slots, a Layered arch up to 8. The customer fills those slots per design type — physics governs the within-shape experience. The Zeigarnik empty-slot mechanic is distinct and operates one level up: in the composition view, uncolored shape silhouettes signal that the customer's event design is incomplete. A customer who has colored an arch sees a greyed column silhouette — not because the arch is unfinished, but because their composition (their event) has a shape-shaped gap. These are different UX moves at different levels and must not be conflated.

The Zeigarnik Effect grounds why composition-level empty slots work: people remember and are drawn to resolve uncompleted tasks [Laws of UX: lawsofux.com/zeigarnik-effect/ — "people remember uncompleted or interrupted tasks better than completed tasks"; ux-bulletin.com/zeigarnik-effect-ux/ — explicitly lists "empty states that reference incomplete workflows" as a Zeigarnik application]. The empty slot is not a recommendation card; it is a gap in the customer's own design. The distinction matters: a recommendation card asks the customer to buy something new; an empty slot tells the customer their own work is unfinished. The second framing is less salesy and more true to the coloring-page frame.
...
**The handoff design is the strongest in the field on this dimension — and Round 2 sharpens it further.** Most design tools stop at the customer's emotional moment and leave the merchant path vague. This design closes both loops on a single screen: the customer gets a memento (the visual card), and Jeff gets a structured data payload that is supplier-call-ready before he speaks to anyone.

Crucially: the payload leads with **catalog color names, not hex codes**. "Empowermint" is a specific balloon SKU. "#A8E6CF" is an approximation I used to render the swatch — the catalog has no official hex values yet. When Jeff calls his supplier, he says "I need Empowermint 11-inch for the main cluster" — not "I need a color that's approximately #A8E6CF." The design card format is `Arch / Main: Empowermint (~#A8E6CF)` — name first, hex as display aid. This inversion is deliberate and correct for Jeff's actual workflow. It also means the inquiry payload is immediately actionable even when hex approximations drift from the real swatch color.

The design card carries catalog color name + approximate hex + region label for every fill region in the composition — which is exactly the information Jeff needs to call his balloon supplier and price the job. The phone number in the card footer ("Or call: (801) 285-0860") respects Jeff's existing contact preference and offers a direct path for customers who don't want to wait on a form. No other contestant's design foregrounds the catalog name as the load-bearing identifier; most lead with hex and treat the name as a display label. This design treats the name as what it actually is: the order SKU.
