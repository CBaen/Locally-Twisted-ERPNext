...
The build IS worth it. The architecture is correct. The cumulative iteration cost is real but is producing a coherent receiving-layer system that matches the behavioral pattern catalog_data demonstrably uses.
...
catalog_data's Classic Arch (live, observed by OpenClaw) has 4 attributes:
...
**Multi-color "combinations" should NOT be SKU variants in either catalog_data or ERPNext.** They should be no-variant structured options preserved on the cart/order line. The "10,000+ combo SKUs" framing was the wrong problem.

The LT custom app is already implementing this pattern. OpenClaw's `erpnext-receiving-build-spec-from-catalog_data-2026-05-10.md` documents the existing alignment:
...
| **E. Keep catalog_data's storefront, decommission rest of catalog_data** | catalog_data storefront has the depth. | Build catalog_data↔ERPNext sync layer for orders/customers/inventory. Two systems, two admins, ongoing catalog_data upgrade maintenance. | **High.** Inverts the project's stated direction. catalog_data failed in testing for reasons that may not be storefront-specific. | Reject. |
...
- **Lane A catalog_data source map** was missing initially (now filled by OpenClaw's `catalog_data-source-commerce-map-2026-05-10.md` + the live backend witness). Process discipline: no artifact, no evidence.
...
- `ground-truth-findings.md` (34KB — local LT codebase + catalog_data clone benchmark)

**OpenClaw's parallel expedition (`workstreams/ecommerce-audit/`):**
- `README.md` — evidence inventory
- `catalog_data-source-commerce-map-2026-05-10.md` — Lane A catalog_data source map
...
- `catalog_data-docs-agent-action-convergence-2026-05-10.md` — Lane E convergence
- `catalog_data-backend-architecture-and-checkout-logic-2026-05-10.md` — live catalog_data backend witness
...
- `erpnext-receiving-build-spec-from-catalog_data-2026-05-10.md` — concrete build spec
...
- **Phase 2 (Convergence):** Substantively performed by OpenClaw's `catalog_data-docs-agent-action-convergence-2026-05-10.md` plus the explicit cross-source agreement between my 3 researcher findings and OpenClaw's audit packets. Running a formal convergence agent would be triangulation-of-triangulation.
