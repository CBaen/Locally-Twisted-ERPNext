...
- Color handling here: colors are captured as `color_recipes` (multi-value arrays per axis), not collapsed into single-SKU mapping. This is the correct multi-color path for Odoo-style color combos.
...
**Confirmed data state:** 95 source extra images across 49 products are unclassified. No `Website Slideshow` records exist for approved parent-gallery media. Source: `audits/odoo-erpnext-migration-audit-2026-05-08/23-product-page-media-classification-packet.md` and `20-product-page-media-visibility-report.md`.
...
**Price state:**
- 13 bouquet templates repaired with per-size prices (Small $35 / Medium $70 / Large $85) via Odoo `get_combination_info` scraper (`repair_variant_prices_from_odoo.py`).
- 36 non-bouquet templates still have live-snapshot prices (single price copied from Odoo page base price at scrape time — not per-variant price deltas). 273 live-snapshot-priced sale units in the review packet, 0 approved for public pricing.
...
- **What:** Original `scrape_odoo_live.py` copied the page's base/display price into every variant, creating flat pricing (e.g., all Unicorn Bouquet sizes showing $35 instead of Small $35 / Medium $70 / Large $85).
- **When:** Commit `c7f9da3` on 2026-05-08.
- **Outcome:** Added `repair_variant_prices_from_odoo.py` which hits Odoo's `website_sale/get_combination_info` route per combination to get correct price deltas. Fixed 13 bouquet templates. 36 non-bouquet templates still pending — these need the same resolver treatment but haven't been run yet.
- **Relevance now:** The repair approach works (calling live Odoo's `combination_info` endpoint). The 36 remaining templates are a known gap, not a structural impossibility.
...
Odoo's `website_sale` manages variant pricing via `price_extra` on `product.template.attribute.value` — a delta per attribute value. ERPNext requires a separate `Item Price` record per full variant combination (item code). With 10,227 active variants across 53 templates, every price delta in Odoo needs a distinct `Item Price` row in ERPNext.

The `repair_variant_prices_from_odoo.py` script resolves this by calling Odoo's `get_combination_info` for each required combination. 13 templates done; 36 remaining. This is implementation work, not a platform architectural barrier.
...
Odoo's `website_sale` also has no native per-variant image gallery in its upstream source — variant images are stored on `product.product` records (one image per variant), which is functionally equivalent to ERPNext's `Item.image`. The Odoo LT custom module adds a backend Images tab via `product_template_image_ids` (template-level gallery), but `deploy.py` does NOT export these extra images — they exist in Odoo admin but are not in the scraped catalog.
...
36 templates still have scraper-copied base prices on all variants instead of correct per-size/per-attribute deltas. `repair_variant_prices_from_odoo.py` can fix these by calling Odoo's `get_combination_info`. This is implementation backlog, not a data model impossibility.
...
## Odoo `website_sale` Benchmark — What It Actually Has

This is the implicit benchmark GL refers to when saying "Odoo had so much more depth."

**Per-variant images:** Stored on `product.product` records (one image per variant). The `deploy.py` catalog exporter captures only `image_1920` on `product.template` (primary product image), NOT `product.product` variant images AND NOT `product_template_image_ids` extra gallery images. This means even Odoo's own export pipeline didn't include the full variant-image set. Equivalent to ERPNext's `Item.image`.

**Combination info / dynamic pricing:** `website_sale/get_combination_info` is Odoo's JSON-RPC endpoint that returns price, availability, and image URL for a specific attribute combination. LT already reverse-engineered and uses this at `ODOO_COMBINATION_ROUTE` in `repair_variant_prices_from_odoo.py`. ERPNext's equivalent is `get_next_attribute_and_values` which returns `product_info.price.formatted_price_sales_uom` for the exact match.

**Color swatch rendering:** Odoo's `website_sale.variants` template renders `html_color` from `product.attribute.value` as colored dots. LT's `website_sale_templates.xml` (the Odoo custom module) overrides this to use `ptav.html_color` for color dots on multi-checkbox attributes. ERPNext has no native `html_color` on Item Attribute Value — LT's `color_rules.py` approximates this with the Qualatex hex map. Functionally equivalent, but Odoo's `html_color` is operator-editable per attribute value; LT's hex values are hardcoded in Python.

**Conditional attribute display / multi-attribute variant navigation:** Odoo uses native `product.template.attribute.line` with `create_variant` flag to distinguish variant-generating axes from non-variant multi-select axes. `display_type` on the attribute controls radio vs. multi-checkbox vs. color pill rendering. ERPNext has no equivalent `display_type` on Item Attribute — all attributes are treated as variant-generating axes. LT's `addon_rules.py` and `color_rules.py` provide a code-level equivalent by classifying axes as required variant / optional add-on / color customization.

**Cart:** Odoo's built-in cart is server-side, requires login or creates a guest session. LT's cart is localStorage-backed guest cart (custom). Functionally LT's cart is more capable for guests.

**Checkout:** Odoo's checkout uses its native payment provider stack. LT's checkout uses Stripe Checkout Sessions (modern, hosted). LT's checkout is more capable (guest checkout, Utah tax resolution, pickup/delivery logic).

**The honest comparison:** Odoo's `website_sale` is more mature and battle-tested. The main concrete depth advantage is that `product.attribute.value.html_color` is operator-configurable in the Odoo admin, while LT's hex values are hardcoded in Python and require a code deploy to add new colors. Everything else is either matched or exceeded by LT's custom implementation.
...
4. Odoo's own `deploy.py` export script did NOT include variant images or template gallery images — the depth advantage GL remembers from Odoo's admin UI is not reflected in Odoo's own export tooling.
