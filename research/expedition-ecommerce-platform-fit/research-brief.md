...
**Phase 1 demo:** Jeff opens the new site, picks a few products with colors/sizes, confirms the workflow feels like the business he runs. He sees something noticeably better than the failed Odoo attempt — without being told the prior attempt failed.
...
- **Variant media swap partially built:** 1,712 variants have `Item.image` mapped from Odoo source; 95 source extra images still unclassified; gallery destination records missing.
...
- Phase 1 demo stealth: Jeff has not been told the prior Odoo attempt failed in testing.

## Destructive Boundaries

- Do NOT modify any file in `C:\Users\baenb\projects\locally-twisted-odoo\` (read-only reference per standing rule 2026-04-25).
...
- **Bulk Odoo→ERPNext catalog import (2026-04-30):** rejected; product-family-by-family approach landed.
...
- **Variant price flatness (2026-05-08):** original scraper copied page base price into each variant row. Fixed for bouquets via Odoo dynamic resolver; 36 non-bouquet templates still need per-family audit.
...
- **C. Clone-from-source.** Read another platform's variant + photo + color-combo + cart source (Odoo's `website_sale` is the obvious benchmark, but Saleor/Sylius/etc. are also clonable), port specific UI components and logic to a custom Jinja-rendered storefront on top of ERPNext primitives. No Webshop.
...
- **E. Keep Odoo's storefront only.** Decommission rest of Odoo, run Odoo's website module as the storefront; orders/customers/inventory sync to ERPNext via API.
...
- `workstreams/odoo-erpnext-ecommerce-parity-research-brief.md` — prior research framing
