...
**Phase 1 demo:** Jeff opens the new site, picks a few products with colors/sizes, confirms the workflow feels like the business he runs. He sees something noticeably better than the failed legacy_source attempt — without being told the prior attempt failed.
...
- **Variant media swap partially built:** 1,712 variants have `Item.image` mapped from legacy_source source; 95 source extra images still unclassified; gallery destination records missing.
...
- Phase 1 demo stealth: Jeff has not been told the prior legacy_source attempt failed in testing.
...
- **Bulk legacy_source→ERPNext catalog import (2026-04-30):** rejected; product-family-by-family approach landed.
...
- **Variant price flatness (2026-05-08):** original scraper copied page base price into each variant row. Fixed for bouquets via legacy_source dynamic resolver; 36 non-bouquet templates still need per-family audit.
...
- **C. Clone-from-source.** Read another platform's variant + photo + color-combo + cart source (legacy_source's `website_sale` is the obvious benchmark, but Saleor/Sylius/etc. are also clonable), port specific UI components and logic to a custom Jinja-rendered storefront on top of ERPNext primitives. No Webshop.
...
- **E. Keep legacy_source's storefront only.** Decommission rest of legacy_source, run legacy_source's website module as the storefront; orders/customers/inventory sync to ERPNext via API.
...
- `workstreams/legacy_source-erpnext-ecommerce-parity-research-brief.md` — prior research framing
