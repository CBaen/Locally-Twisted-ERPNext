...
- `_resources/odoo-live/catalog.json` and `_resources/odoo-live/slug_to_group.json` - Odoo source export still says `product_count: 53`.
- `audits/odoo-erpnext-migration-audit-2026-05-08/current-state-snapshot-2026-05-17-2132-clean-odoo-products/summary.json` - read-only snapshot with `website_items: 53`, `items: 10674`, `item_prices: 10656`.
...
- `workstreams/ecommerce-audit/odoo-sellable-product-reimport-2026-05-17.md` - says 53 included products, 0 exclusions, browser proof for all 53, with later holds for unsafe direct checkout.
...
- Source-vs-DB compare:
  - `_resources/odoo-live/catalog.json` has `product_count = 53` and 53 source slugs.
...
- **Eliminate:** Eliminate unqualified hard-coded product counts in docs and tests. `53` may remain only as "Odoo source export count" or "historical snapshot count"; `51` may remain only as "current local DB count as of 2026-05-21" unless rechecked.
...
- The Odoo source export still says 53 and `catalog_import_subset.py` has no owner-excluded slugs, so current source and current DB are not reconciled.
