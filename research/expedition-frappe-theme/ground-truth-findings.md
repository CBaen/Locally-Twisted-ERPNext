# Ground Truth Findings: LT Codebase + Frappe Container + Odoo Source
## Date: 2026-04-26
## Source Type: Codebase, Git History, Container Inspection, Odoo Source XML
## Project: Locally Twisted
## Files Examined: 47

---

## Approved Jeff-Vetted Content From Local Odoo Source

This is the most load-bearing section. Every string below was pulled verbatim from XML in `C:/Users/baenb/projects/locally-twisted-odoo/addons/locally_twisted/views/`. The Odoo project contains far more view files than the brief anticipated. Full list of XML files found (43 total):
...
**NOTE:** The footer does NOT include a Twitter/X social link. The `setup_slice2_header_footer.py` script added Twitter — that was wrong per the Odoo source.
...
- Photo carousel: 10 slides, Odoo ir.attachment IDs 1209–1227 (odd numbers alternating twisting/face painting). These are production Odoo attachment IDs — NOT transferable directly to ERPNext.
...
Note: These use Odoo category slugs with Odoo numeric IDs. ERPNext Item Group routes will be different.
...
- Photo carousel: 10 slides, Odoo ir.attachment IDs 1188–1197
...
- Photo carousel: 9 slides, Odoo ir.attachment IDs 1198–1206
...
**Approach 2 — `setup_pages/landing.py` `build()` function:** Built a Web Page record with `content_type="Page Builder"` and 4 Frappe default Web Templates ("Hero with Right Image", "Section with Cards", "Section with CTA"). Used invented copy (not Odoo-sourced). Page not visible to GL when opened in their real browser. Not mobile-responsive. Rolled back to placeholder via `rollback()`. The `build()` function is kept for reference but marked NOT TO RUN.
...
**File:** `C:/Users/baenb/projects/Built_by_Cameron/_CLIENTS/locally-twisted/_resources/odoo-export/catalog.json`

**Source:** `http://5.78.136.133` (live Odoo), category `/shop/category/what-we-make-3`
...
**Images:** image_url format is `http://5.78.136.133/web/image/product.product/{id}/image_1920?unique={hash}` — these are Odoo attachment URLs, NOT local files. They require fetching from the live Odoo server. Live Odoo stack is still running (`locally-twisted-odoo-web-1 Up 30 hours`).

**Description:** Most products have `description: null` (no descriptions in the Odoo catalog).
...
**Footer social icons:** Only 3 (Facebook, Instagram, Pinterest). The `setup_slice2_header_footer.py` script erroneously added Twitter. Odoo source is authoritative.
...
### Odoo image URLs are not local
The catalog.json images are Odoo server URLs (`http://5.78.136.133/...`). The local Odoo stack is currently running, so they're fetchable now. But when the Odoo server is decommissioned (planned after ERPNext replacement is ready), all 48 image URLs become 404. An export-before-decommission step is essential.
...
2. **Hero images do not exist locally** — the `s_lt_hero.xml` hero carousel references images by Odoo attachment IDs (production DB records). No hero image files exist in `_resources/images/`. The `_resources/images/` folder contains 15 AI-generated placeholder PNGs, but none appear to be hero carousel slides.

3. **Trust bar SVG icons are not in the ERPNext app** — `trust-trophy.svg`, `trust-palette.svg`, `trust-heart.svg` are referenced at `/locally_twisted/static/src/img/` (Odoo path). The ERPNext app's `public/icons/` directory exists but these specific SVGs have not been copied over.
...
5. **Pricing calculator has no implementation** — the BTFP service page plan calls for a pricing calculator in the `javascript` field, but no implementation exists. The Odoo version (`page_balloon_twisting.xml`) also has no calculator — it has a booking form, not a pricing calculator.
...
- All hero images are Odoo attachment URL references, not local files

**What is fully ready (from Odoo source):**
...
- This copy has been through the Odoo build process and represents the Jeff-approved content intent

### Path of least resistance for the landing page

The approved Odoo content maps cleanly to Frappe's native Web Page Page Builder PLUS some custom HTML in the `header` field or `main_section_html`. Specifically:
...
### What the approved Odoo content tells us about Jeff's actual taste

The Odoo codebase was built carefully — it has WCAG-correct aria attributes, accessibility structure, Bootstrap 5 syntax, and Quiet Confidence voice. The hero copy ("Utah's Balloon Specialists", "Making celebrations unforgettable since 1998") IS on-brand per the STYLE-GUIDE. The CTA copy ("Make Your Celebration Unforgettable") IS on-brand. The client crawl list (52 names including Utah Jazz, Ancestry, University of Utah) tells a strong social proof story.
