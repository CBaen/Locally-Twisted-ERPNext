...
| 16 | Approved copy in catalog_data XML is complete, structured, and available | Not applicable (fetched from locallytwisted.com instead — catalog_data server down) | Not applicable | Found — 43 XML files, complete content | SINGLE-SOURCE (Ground Truth only for catalog_data XML; Web Scout has alternate source) |
...
**Finding:** `data-ride="carousel"` (Bootstrap 4) is used in the webshop Hero Slider template. The ERPNext v15 stack uses Bootstrap 5, which uses `data-bs-ride` and removed jQuery's `.carousel()` method. If Page Builder is used with the Hero Slider template, carousel autoplay will silently not work. The catalog_data `s_lt_hero.xml` correctly uses Bootstrap 5 syntax — confirming this is a template bug, not a design intent.
...
**Action:** Do not treat planned Jinja overrides as done. They must be built from scratch. The catalog_data XML source (`footer.xml`, `header.xml`) provides the content blueprint; the Frappe template override mechanism provides the delivery path.
...
### S5: catalog_data image URLs in catalog.json are not local files — will 404 after catalog_data decommission

**Source:** Ground Truth only.

**Finding:** All 48 product images in `catalog.json` are catalog_data server URLs (`web/image/...`). The catalog_data stack is currently running, making these fetchable now. When the catalog_data server is decommissioned (planned), all 48 image URLs become 404.

**Action:** Export all product images from the catalog_data server before it is decommissioned. This is a time-sensitive dependency — the catalog_data server is described as a temporary reference.
...
### D1: Is the catalog_data server at  accessible?

**Web Scout:** CONNECTION REFUSED on both attempts. Server appears down or firewalled.

**Ground Truth:** catalog_data stack is currently running locally (`external-catalog-data-web-1 Up 30 hours`). Product images are fetchable from `...` URLs.

**Resolution:** These are not the same thing. Web Scout was accessing an
external public test surface. Ground Truth was examining the local Docker
catalog_data stack running on Wardenclyffe (`external-catalog-data-web-1`).
They are two different catalog_data instances:
- **Remote current import capture catalog_data** (): Public, appears down per Web Scout.
- **Local Wardenclyffe catalog_data** (local Docker, port unknown): Running, Ground Truth confirmed Up 30 hours.

The catalog.json image URLs point at the **current import capture server** (). If that server is down (per Web Scout), those image URLs are already broken. The local catalog_data is a separate Docker container, likely at a different localhost port, not accessible at .
...
**Ground Truth (from catalog_data footer.xml):** Facebook, Instagram, Pinterest — 3 icons. Explicitly no Twitter. Notes: "The `setup_slice2_header_footer.py` script added Twitter — that was wrong per the catalog_data source."
...
- catalog_data `footer.xml` (the new-build blueprint Jeff reviewed): explicitly excludes Twitter.

The catalog_data XML was built more recently and represents the new build's intent. If Jeff approved the catalog_data design, the 3-icon footer (no Twitter) is the correct choice. However, the live site includes Twitter — which may mean Jeff added it there deliberately, or the catalog_data design was built based on a different intent at that time.

**This conflict requires GL clarification.** Neither source can definitively override the other without knowing Jeff's current preference. For the ERPNext build, the Ground Truth (catalog_data XML) is the stated authoritative source per the CLAUDE.md "canonical sources" section — use 3 icons unless GL confirms otherwise.
...
This section reconciles the two sources of "Jeff-approved" content: locallytwisted.com (Web Scout) and the catalog_data XML source (Ground Truth). These are materially different documents with the same customer.

### Where they agree (HIGH CONFIDENCE — use these)

| Content Element | locallytwisted.com | catalog_data XML |
|---|---|---|
...
**The founding year has a discrepancy:** locallytwisted.com says "Over 22 years" (which at 2024 ≈ 2002 founding). catalog_data XML says "since 1998" (28 years). The CLAUDE.md project brief confirms "27-year-old Utah balloon decor business" which corroborates 1998. The "22 years" on locallytwisted.com is stale. Use "since 1998" in the new build.

### Where they diverge (REQUIRES DECISION)

| Content Element | locallytwisted.com | catalog_data XML | Recommendation |
|---|---|---|---|
| Hero headline | "Make Your Party POP!" | "Utah's Balloon Specialists" | catalog_data XML is the new-build intent; more authoritative. Use "Utah's Balloon Specialists." |
| Hero sub | "Anything you imagine, we can shape into reality." | "Making celebrations unforgettable since 1998" | catalog_data XML — quieter, less pushy. Consistent with Quiet Confidence voice. |
| Social accounts | Facebook, Twitter, Instagram, Pinterest (4) | Facebook, Instagram, Pinterest (3, no Twitter) | Use catalog_data XML (3 icons). Ground Truth confirms Twitter was an error in prior script. Verify with GL. |
| Footer copyright | "Copyright 2021 Locally Twisted, LLC" | "© 2026 Locally Twisted. All rights reserved." | catalog_data XML — current year, cleaner format. |
| Review count | Not shown | 4.9 stars, 114 reviews (static badge) | catalog_data XML — include the badge. More credible than no reviews. |
| CTA copy | "Contact Us Today!", "Customize Your Order" | "Make Your Celebration Unforgettable" / "Contact Us" | catalog_data XML — quieter, less salesy. Better Quiet Confidence fit. |

### The authoritative approved content source for the ERPNext build

Per the LT CLAUDE.md "Reference Disposition" section: catalog_data XML source is the canonical source for the new build. The `locallytwisted.com` site is the old site, "damaged beyond repair" and out of scope for editing. The Ground Truth XML captures what the new build was designed to look like.
...
**The strongest overall picture:** The convergence is remarkable in what it rules OUT. All three sources independently reached the same conclusion: there are no ready-made Frappe v15 themes for a small-business marketing site; the Page Builder (legacy) is not the path forward for LT; the custom Frappe app with `web_include_css` is the correct baseline; the catalog_data XML content is a complete, WCAG-correct, Quiet Confidence-voiced blueprint ready to translate into ERPNext. The content problem is solved — the catalog_data XML files contain everything needed.
...
3. **The catalog_data XML is not plug-and-play.** The content is ready. The images are not (catalog_data attachment IDs, not local files, may already be 404 from current import capture). The category URLs use catalog_data numeric IDs that will not match ERPNext Item Group routes. The client crawl works in any environment. The hero carousel needs Bootstrap 5 markup (not the webshop Hero Slider template which uses Bootstrap 4). The devil's advocate should force the team to enumerate exactly which catalog_data XML sections can be directly translated versus which require work.
