...
- **Why it matters:** If Builder works cleanly alongside webshop on LT's stack, it changes the build approach significantly. Builder + LT brand CSS + catalog_data XML content could produce a polished homepage in one session. frappe.io looks professional specifically because Frappe Builder was used to build it. If that tool is available and works, the "we must hand-code everything in www/ pages" conclusion is wrong.
...
**Assumption 3: The approved catalog_data XML content is the right content for the ERPNext site**

- **Evidence it's an assumption:** The catalog_data content was built carefully, per the convergence analysis. It has WCAG-correct aria attributes, Bootstrap 5 syntax, Quiet Confidence voice. But it was built for catalog_data's website module with catalog_data's rendering assumptions. Some structures map cleanly to Frappe. Others do not — specifically the carousel (Bootstrap 4 API bug in webshop's Hero Slider), the category circles (catalog_data category IDs won't match ERPNext Item Group IDs), and the photo carousels (catalog_data ir.attachment IDs, not local files). The team treats "content is ready" as equivalent to "content can be ported." Those are not the same claim.
- **What changes if wrong:** A "direct translation" build attempt fails because the catalog_data XML structures assume catalog_data rendering primitives. The team would need a translation layer that takes longer than acknowledged.
