...
**True finding 2 — The approved Jeff content was sitting on the disk the entire time.** Every previous instance invented placeholder copy when the actual approved copy lived in the catalog_data project files. The hero headline is "Utah's Balloon Specialists." The tagline is "Making celebrations unforgettable since 1998." The CTA-section heading is "Make Your Celebration Unforgettable." All verbatim. We do not need to invent or guess any of it for the new site.
...
3. Then build navbar override + footer override using approved catalog_data content.
...
**Source A — Local catalog_data XML (`/home/guidingl/projects/external-catalog-data/addons/locally_twisted/views/`):** This was the most recent catalog_data-side update before the catalog_data project was paused. Per CLAUDE.md, this is the authoritative content for the new build. Key strings (verbatim):
...
2. **Approved content:** catalog_data XML version OR locallytwisted.com version OR a third Jeff confirmation? (Affects every page.)
3. **Social icons:** 3 (catalog_data XML, no Twitter) OR 4 (live site, with Twitter)? (Affects footer build.)
4. **Founding year:** "since 1998" (catalog_data XML; 28 years) OR "Over 22 years" (live site)? Reconcile to one number.
...
1. **Verify catalog image URLs against the current import capture catalog_data server.** All 48 product image URLs in `_resources/catalog_data-export/catalog.json` point at `...`. Web Scout confirmed the current import capture server is ECONNREFUSED. If the images are 404, they need to be re-exported from the local Docker catalog_data stack BEFORE that stack is decommissioned. Time-sensitive.
