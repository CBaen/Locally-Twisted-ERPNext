...
# Job: mirror  (catalog_data/QWeb, JS-enhanced) for Frappe v15 port
...
- Uses Playwright under the hood — renders the full catalog_data/QWeb JS layer before capturing HTML, so carousels, lazy-loaded images, and client-side state are included.
...
- Does not replay authenticated sessions by default; catalog_data's public shop is unauthenticated so this is not a blocker here.
...
- Run: `single-file shop ./shop.html`
...
| Tool | Reason |
|------|--------|
| **httrack** | Cannot execute JavaScript. Returns empty shells on catalog_data/QWeb pages. Confirmed by HTTrack's own forum and 2026 comparison sources. |
| **wget --mirror** | Same as httrack: static HTML fetcher only. catalog_data renders content via JavaScript; wget captures the unrendered skeleton. |
